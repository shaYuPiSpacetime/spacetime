-- =====================================================
-- PRD-02 关系反馈与互动链路生产增量迁移
-- 可重复执行；不删除历史关系事实，不写入演示关系数据。
-- =====================================================
...
SET @ddl = IF((SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=@schema_name AND TABLE_NAME='app_user' AND COLUMN_NAME='anonymous_no')=0,
    'ALTER TABLE app_user ADD COLUMN anonymous_no VARCHAR(32) DEFAULT NULL COMMENT ''注销关系匿名编号，前缀ANON-'' AFTER account_status', 'SELECT 1');
...
PREPARE stmt FROM @ddl; EXECUTE stmt; DEALLOCATE PREPARE stmt;
SET @ddl = IF((SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=@schema_name AND TABLE_NAME='app_user_unlock_record' AND COLUMN_NAME='target_biz_no')=0,
    'ALTER TABLE app_user_unlock_record ADD COLUMN target_biz_no VARCHAR(64) DEFAULT NULL COMMENT ''具体关系业务编号，LIK-喜欢记录，VIS-访客记录'' AFTER target_biz_type', 'SELECT 1');
PREPARE stmt FROM @ddl; EXECUTE stmt; DEALLOCATE PREPARE stmt;
SET @ddl = IF((SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=@schema_name AND TABLE_NAME='app_user_unlock_record' AND COLUMN_NAME='refund_no')=0,
    'ALTER TABLE app_user_unlock_record ADD COLUMN refund_no VARCHAR(64) DEFAULT NULL COMMENT ''特批退款业务编号'' AFTER target_biz_no', 'SELECT 1');
PREPARE stmt FROM @ddl; EXECUTE stmt; DEALLOCATE PREPARE stmt;
SET @ddl = IF((SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=@schema_name AND TABLE_NAME='app_user_unlock_record' AND COLUMN_NAME='active_marker')=0,
    'ALTER TABLE app_user_unlock_record ADD COLUMN active_marker TINYINT DEFAULT 1 COMMENT ''有效唯一标记：1-权益有效，NULL-权益已结束'' AFTER refund_no', 'SELECT 1');
PREPARE stmt FROM @ddl; EXECUTE stmt; DEALLOCATE PREPARE stmt;

-- 历史解锁只规范财务状态和业务编号；无法映射具体关系者保持 target_biz_* 为空。
UPDATE app_user_unlock_record SET status='active' WHERE UPPER(status)='ENABLED';
UPDATE app_user_unlock_record SET status='expired' WHERE UPPER(status)='DISABLED';
UPDATE app_user_unlock_record SET unlock_no=CONCAT('ULK-LEGACY-', LPAD(id, 18, '0')) WHERE unlock_no IS NULL OR unlock_no='';
UPDATE app_user_unlock_record SET active_marker=NULL WHERE status IN ('expired', 'refunded');
ALTER TABLE app_user_unlock_record MODIFY COLUMN unlock_no VARCHAR(64) NOT NULL COMMENT '解锁业务编号，前缀ULK-';
ALTER TABLE app_user_unlock_record MODIFY COLUMN status VARCHAR(20) NOT NULL DEFAULT 'active' COMMENT '解锁状态：active-有效，expired-已过期，refunded-已退款';

-- 注销用户稳定匿名号；不输出手机号、openid 等个人身份信息。
UPDATE app_user
SET anonymous_no=CONCAT('ANON-', UPPER(SUBSTRING(REPLACE(UUID(), '-', ''), 1, 16)))
WHERE account_status IN ('CANCELLING', 'CANCELLED') AND (anonymous_no IS NULL OR anonymous_no='');

SET @ddl = IF((SELECT COUNT(*) FROM information_schema.STATISTICS WHERE TABLE_SCHEMA=@schema_name AND TABLE_NAME='app_user' AND INDEX_NAME='uk_app_user_anonymous_no')=0,
    'ALTER TABLE app_user ADD UNIQUE KEY uk_app_user_anonymous_no (anonymous_no)', 'SELECT 1');
PREPARE stmt FROM @ddl; EXECUTE stmt; DEALLOCATE PREPARE stmt;
