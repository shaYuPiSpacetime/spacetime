...
const statuses = opts([['sent','已发送'],['queued','待发送'],['pending','等待回应'],['replied','已回应'],['expired','已过期'],['invalid','已失效'],['unread','未读'],['read','已读'],['failed','失败']]);
const messageTypes = opts([['text','文本'],['whisper','悄悄话'],['whisper_reply','悄悄话回复'],['system','系统消息'],['system_tip','系统提示'],['assistant','官方助手']]);
const systemCategories = opts([['governance','治理'],['asset','资产'],['invite','邀请'],['community','社区运营'],['platform','平台安全'],['assistant','官方助手']]);
...
export default function MessageRecordPage() {
  ...
  useEffect(()=>{if(!filterReadyRef.current){filterReadyRef.current=true;return;}const timer=window.setTimeout(()=>setQuery({...filters,page:1,startTime:dateTime(filters.startTime),endTime:dateTime(filters.endTime,true)}),300);return()=>window.clearTimeout(timer);},[filters.keyword,filters.recordType,filters.messageType,filters.systemCategory,filters.status,filters.startTime,filters.endTime]);
  ...
  const closeDetail=useCallback(()=>{setDetailOpen(false);setDetail(null);setSensitive(null);setReason('');},[]);
  ...
  return <CommunityPage>
    ...
    <FilterPanel busy={loading} showSearch={false} onSearch={()=>{}} onReset={()=>{setFilters(initial);setQuery(initial);}}>
    ...
    <Drawer open={detailOpen} onClose={closeDetail} title="消息记录详情" description={detail?.recordNo}>{detailLoading?<p>加载中...</p>:detail&&<div className="space-y-4"><DetailGrid items={[{label:'记录类型',value:recordTypes.find(x=>x.code===detail.recordType)?.label||detail.recordType},{label:'消息类型',value:messageTypes.find(x=>x.code===detail.messageType)?.label||detail.messageType},{label:'用户',value:userText(detail.userId,detail.userNickname)},{label:'对方用户',value:userText(detail.peerUserId,detail.peerNickname)},{label:'状态',value:statuses.find(x=>x.code===detail.status)?.label||detail.status},{label:'会话编号',value:detail.conversationNo},{label:'来源业务编号',value:detail.sourceBizNo},{label:'失败原因',value:detail.failureReason},{label:'关联举报',value:detail.caseCount}]}/>{detail.sensitiveContent?<DetailSection title="消息正文"><Button variant="outline" disabled={!canSensitive||!detail.contentAvailable} onClick={()=>setSensitiveOpen(true)}><Eye className="mr-1.5 h-4 w-4"/>{!detail.contentAvailable?'正文不可用':canSensitive?'查看高敏正文':'无查看权限'}</Button></DetailSection>:<DetailSection title={detail.title||'消息内容'}><div className="whitespace-pre-wrap break-words">{detail.content||'-'}</div></DetailSection>}</div>}</Drawer>
