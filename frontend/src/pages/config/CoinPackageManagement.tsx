...
export default function CoinPackageManagement() {
  const [list, setList] = useState<CoinPackage[]>([]);
  ...
  function openEdit(row: CoinPackage) {
    setEditing(row);
    setForm({
      packageName: row.packageName,
      amount: String(row.amount ?? ''),
      coinCount: String(row.coinCount ?? ''),
      bonusCoinCount: String(row.bonusCoinCount ?? ''),
  ...
  async function handleSave() {
    if (!form.packageName.trim() || !form.amount) return;
    setSaving(true);
    try {
      const data = {
        packageName: form.packageName.trim(),
        amount: Number(form.amount),
        coinCount: Number(form.coinCount || 0),
        bonusCoinCount: Number(form.bonusCoinCount || 0),
        recommendFlag: Number(form.recommendFlag || 0),
        packageTag: form.packageTag.trim() || undefined,
        packageDesc: form.packageDesc.trim() || undefined,
        sortOrder: Number(form.sortOrder || 0),
        status: form.status,
      };
      if (editing) {
        await updateCoinPackage(editing.id, data);
      } else {
        await createCoinPackage(data);
      }
      setDialogOpen(false);
      fetchList();
    } finally {
  ...
  async function toggleStatus(row: CoinPackage) {
    const nextStatus = row.status === 'ENABLED' ? 'DISABLED' : 'ENABLED';
    await updateCoinPackageStatus(row.id, nextStatus);
    fetchList();
  }
  ...
  const filteredList = keyword
    ? list.filter(
        (item) =>
          ...
          (item.packageDesc && item.packageDesc.includes(keyword)),
      )
    : list;
    ...
    <div className="space-y-4">
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>到账币数</TableHead>
                <TableHead>赠送币数</TableHead>
                <TableHead>标签</TableHead>
                <TableHead>优惠说明</TableHead>
              ...
              {loading ? (
                ...
                filteredList.map((row) => (
                  <TableRow key={row.id}>
                    ...
                    <TableCell>{row.coinCount}</TableCell>
                    <TableCell>
                      {row.bonusCoinCount ? (
                        <span className="text-green-600">+{row.bonusCoinCount}</span>
                      ) : (
                        '-'
                      )}
                    </TableCell>
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>维护成家币充值套餐的配置信息</DialogDescription>
        </DialogHeader>
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <label className="space-y-1 text-sm font-medium">
              ...
              <Input
                ...
                onChange={(e) => setForm({ ...form, coinCount: e.target.value })}
                placeholder="如：60"
              />
            </label>
            <label className="space-y-1 text-sm font-medium">
              ...
              <Input
                type="number"
                value={form.bonusCoinCount}
                onChange={(e) => setForm({ ...form, bonusCoinCount: e.target.value })}
                placeholder="如：6"
              />
            </label>
          </div>
