...
export default function ProfileEditSongsPage() {
  ...
  const save = async (song: SongOption) => { if (savingId) return; setSavingId(song.songId); try { await prd01Api.saveFavoriteSong(song); await Taro.showToast({ title: '保存成功', icon: 'success' }); navigateBackOrRedirect() } catch (error) { await showError(error) } finally { setSavingId('') } }
  return <View style={{ minHeight: '100vh', background: '#F3F7FB' }}><LanhuSubNav title="我最爱听的歌曲" onBack={navigateBackOrRedirect} /><View style={{ width: '700rpx', height: '92rpx', borderRadius: '18rpx', background: '#FFFFFF', margin: '30rpx auto', padding: '0 24rpx', display: 'flex', alignItems: 'center', boxSizing: 'border-box' }}><Input value={keyword} placeholder="搜索歌曲或歌手" onInput={event => setKeyword(event.detail.value)} onConfirm={() => void search()} style={{ flex: 1, fontSize: '29rpx' }} /><Text style={{ color: '#2876FF', fontSize: '28rpx', padding: '20rpx' }} onClick={() => void search()}>搜索</Text></View><ScrollView scrollY style={{ height: 'calc(100vh - 300rpx)' }}><View style={{ width: '700rpx', margin: '0 auto', borderRadius: '20rpx', background: '#FFFFFF', padding: '14rpx 26rpx', boxSizing: 'border-box' }}>{songs.map(song => <View key={song.songId} style={{ minHeight: '116rpx', borderBottom: '2rpx solid #F0F2F6', display: 'flex', alignItems: 'center' }} onClick={() => void save(song)}>{song.coverUrl ? <Image src={song.coverUrl} mode="aspectFill" style={{ width: '82rpx', height: '82rpx', borderRadius: '12rpx', marginRight: '20rpx' }} /> : null}<View style={{ flex: 1 }}><Text style={{ display: 'block', color: '#333333', fontSize: '29rpx', fontWeight: 700 }}>{song.songName}</Text><Text style={{ display: 'block', color: '#999999', fontSize: '24rpx', marginTop: '8rpx' }}>{song.artistName || ''}</Text></View><Text style={{ color: '#2876FF', fontSize: '26rpx' }}>{savingId === song.songId ? '保存中...' : '选择'}</Text></View>)}</View></ScrollView></View>
}
