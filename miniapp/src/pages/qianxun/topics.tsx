...
import { getCommunityTopics, type CommunityTopicCardVO } from '@/services/community'
...
export default function QianxunTopicsPage() {
  ...
  const loadPage = async (nextPage: number, reset = false) => {
    ...
    setErrorMessage('')
    try {
      const result = await getCommunityTopics(nextPage, PAGE_SIZE)
  ...
  return <View id="qianxun-topics-page" style={{ height: '100vh', background: '#F4F5F7', overflow: 'hidden' }}>
    ...
    <ScrollView scrollY onScrollToLower={() => void loadPage(page + 1)} lowerThreshold={120} style={{ position: 'absolute', left: 0, right: 0, top: `${metrics.navigationHeight}rpx`, bottom: 0 }} showScrollbar={false}>
      <View style={{ width: '750rpx', padding: '20rpx 25rpx calc(42rpx + env(safe-area-inset-bottom))', boxSizing: 'border-box' }}>
        {topics.map(topic => <TopicListCard key={topic.id} topic={topic} />)}
        {!topics.length && loading ? <TopicListSkeleton /> : null}
        {!topics.length && !loading ? <ListState text={errorMessage || '暂无社区话题'} action={errorMessage ? () => void loadPage(1, true) : undefined} /> : null}
        {topics.length && loading ? <Text style={{ display: 'block', color: '#AAAAAA', fontSize: '22rpx', lineHeight: '72rpx', textAlign: 'center' }}>加载中...</Text> : null}
        {topics.length && errorMessage ? <ListState text={errorMessage} action={() => void loadPage(page + 1)} compact /> : null}
        {topics.length && !hasMore && !loading ? <Text style={{ display: 'block', color: '#B0B0B0', fontSize: '22rpx', lineHeight: '72rpx', textAlign: 'center' }}>已经到底啦</Text> : null}
      </View>
    </ScrollView>
  </View>
}

function TopicListCard({ topic }: { topic: CommunityTopicCardVO }) {
  ...
  return <View onClick={open} style={{ width: '700rpx', minHeight: '278rpx', borderRadius: '16rpx', background: '#FFFFFF', marginBottom: '20rpx', padding: '24rpx 24rpx 22rpx', boxSizing: 'border-box' }}>
    ...
    <View style={{ display: 'flex', marginTop: '18rpx' }}>
      ...
      <View style={{ minWidth: 0, flex: 1, height: '128rpx', marginLeft: '18rpx', display: 'flex', flexDirection: 'column' }}>
        <Text style={{ display: '-webkit-box', color: '#555555', fontSize: '24rpx', lineHeight: '37rpx', height: '74rpx', overflow: 'hidden', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{topic.description || topic.previewContent || '分享与话题有关的真实经历和此刻想法。'}</Text>
        <View style={{ display: 'flex', alignItems: 'center', marginTop: 'auto' }}>
          ...
          <Text style={{ maxWidth: '220rpx', color: '#999999', fontSize: '20rpx', marginLeft: '9rpx', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{topic.previewAuthorName || '社区用户'}</Text>
          <View style={{ flex: 1 }} />
