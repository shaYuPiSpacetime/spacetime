...
test('首登资料页继续共享蓝湖两态按钮，退出登录手机号页使用设计稿矩形获取验证码按钮', () => {
  ...
  assert.match(button, /active \? miniappOssIcons\.loginNextActive : miniappOssIcons\.loginNextDisabled/)
  assert.match(button, /className="login-next-icon"/)
  assert.doesNotMatch(button, /background: active/)
...
test('退出登录手机号页按五态设计稿实现星空首屏、协议弹窗和验证码自动登录', () => {
  ...
  assert.match(phone, /miniappOssMedia\.loginBackgroundVideo/)
  assert.match(phone, /<Video[\s\S]*autoplay[\s\S]*loop[\s\S]*muted[\s\S]*onError=\{\(\) => setVideoUnavailable\(true\)\}/)
  assert.match(phone, /!videoUnavailable && !showAgreement[\s\S]*className="phone-login-brand"/, '动态视频上应叠加与立即使用页一致的品牌切图')
  ...
  assert.match(styles, /\.phone-login-video/)
  assert.match(styles, /\.phone-login-brand/)
