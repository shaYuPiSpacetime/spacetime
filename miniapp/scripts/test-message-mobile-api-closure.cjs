...
test('私信时间按本地日期和相邻消息十分钟间隔显示', () => {
  const { formatPrivateChatTime } = requireDomain('src/domain/messageRuntime.ts')
  const now = new Date(2026, 8, 17, 21, 0)
  assert.equal(formatPrivateChatTime('2026-09-17T09:24:00', undefined, now), '早上 9:24')
  assert.equal(formatPrivateChatTime('2026-09-17T09:33:59', '2026-09-17T09:24:00', now), '')
  assert.equal(formatPrivateChatTime('2026-09-17T09:34:00', '2026-09-17T09:24:00', now), '')
  assert.equal(formatPrivateChatTime('2026-09-17T09:34:01', '2026-09-17T09:24:00', now), '早上 9:34')
  assert.equal(formatPrivateChatTime('2026-09-17T09:39:00', '2026-09-17T09:34:01', now), '')
  assert.equal(formatPrivateChatTime('2026-09-17T00:01:00', '2026-09-16T23:59:00', now), '凌晨 0:01')
  assert.equal(formatPrivateChatTime('2026-09-08T09:34:00', undefined, now), '9月8日 上午 9:34')
  assert.equal(formatPrivateChatTime('2026-09-08T09:45:00', '2026-09-08T09:34:00', now), '9月8日 上午 9:45')
  assert.equal(formatPrivateChatTime('2026-09-08T09:39:00', '2026-09-08T09:34:00', now), '')
  assert.equal(formatPrivateChatTime('invalid'), '')
  assert.equal(formatPrivateChatTime('2026-09-17T09:24:00', 'invalid', now), '早上 9:24')
...
test('私信发送保留键盘，键盘弹出和发送后消息区露出最新消息', () => {
  ...
  assert.match(privateChat, /adjustPosition=\{false\}/)
  assert.match(privateChat, /onKeyboardHeightChange=/)
