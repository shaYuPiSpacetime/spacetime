...
assert.ok(membershipHook.includes("Taro.switchTab({ url: '/pages/profile/index' })"), '会员支付成功或确认中必须跳转到我的页面')
...
assert.ok(membershipPage.includes('MEMBERSHIP_PAY_BAR_RESERVED_RPX'), '会员页必须声明固定支付栏预留高度，避免权益卡片被遮挡')
assert.ok(membershipPage.includes('MEMBERSHIP_CONTENT_BOTTOM_SPACE_RPX'), '会员页滚动内容底部必须保留收尾留白')
assert.ok(membershipPage.includes('height: `calc(100vh - ${MEMBERSHIP_NAV_HEIGHT_RPX + MEMBERSHIP_PAY_BAR_RESERVED_RPX}rpx)`'), '会员页滚动区域高度必须扣除顶部导航和底部支付栏')
