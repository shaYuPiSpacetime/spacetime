...
const verificationEntry = read('miniapp/src/features/verification/VerificationEntryView.tsx')
...
assert.ok(verificationEntry.includes('miniappOssIcons.qianxunCenter'), '认证引导必须引用 OSS 中心插画，避免 526KiB 资源进入主包')
assert.ok(!verificationEntry.includes("@/assets/lanhu/pages/qianxun-center.png"), '认证引导不得直接打包中心插画')
...
assert.ok(!membershipPage.includes('/pages/membership/subscription'), '会员中心不得保留连续订阅管理入口')
assert.ok(!membershipPage.includes('连续订阅会员服务协议'), '一次性购买不得展示连续订阅协议')
assert.ok(!exists('miniapp/src/pages/membership/subscription.tsx'), '连续订阅管理页必须退役')
assert.ok(!exists('miniapp/src/pages/membership/subscription.config.ts'), '连续订阅管理页配置必须退役')
assert.ok(membershipPage.includes("memberStatus === 'active') return '再次购买'"), '已开通会员必须使用一次性再次购买文案')
...
assert.ok(paymentServiceImpl.includes('resolveWechatPaymentAmount(payAmount)'), '微信下单必须通过受控金额解析器使用套餐支付金额')
assert.ok(/if \(!wechatPayProperties\.isForceTestAmount\(\)\) \{\s*return packageAmount;/.test(paymentServiceImpl), '未启用强制测试金额时微信下单必须回落套餐真实支付金额')
...
assert.ok(applicationProd.includes('force-test-amount: ${WECHAT_PAY_FORCE_TEST_AMOUNT:false}'), 'prod 测试支付金额开关必须显式默认关闭')
assert.ok(!applicationProd.includes('force-test-amount: true'), 'prod 不得默认启用测试支付金额覆盖')
