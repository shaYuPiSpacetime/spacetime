...
assert.ok(sharedPicker.includes("background: 'transparent'"), '统一选择弹窗必须移除选项区灰色背景')
assert.ok(sharedPicker.includes('<OptionColumn options={options}'), '单列弹窗不得重复展示字段标题')
...
assert.ok(!realNameSource.includes('bottomAction='), '实名认证提交按钮必须位于表单下方，禁止固定在屏幕底部')
assert.ok(realNameSource.includes("top: '682rpx'"), '实名认证提交按钮纵坐标必须对齐蓝湖表单下方位置')
...
assert.ok(!realNameSource.includes("optionLabel('auditStatus'"), '实名认证页不得展示“未提交”等审核状态条')
assert.ok(realNameSource.includes("label={copy('real_name_name_label')}"), '实名认证输入区必须展示蓝湖稿中的姓名标签')
assert.ok(realNameSource.includes("label={copy('real_name_id_label')}"), '实名认证输入区必须展示蓝湖稿中的身份证号标签')
...
const nativeNavigationSource = read('src/components/NativeNavigation.tsx')
...
assert.ok(subNavSource.includes('NativeNavigation'), '二级页导航必须复用统一系统菜单对齐组件')
assert.ok(nativeNavigationSource.includes('getMenuButtonBoundingClientRect'), '二级页导航必须按微信胶囊位置垂直对齐')
assert.ok(nativeNavigationSource.includes('getWindowMetrics'), '二级页导航必须使用窗口宽度换算 rpx')
...
assert.ok(profileTopNavSource.includes('<MiniappBackIcon color="#607086" size={22} />'), '编辑资料顶部返回箭头必须复用统一蓝湖尺寸组件')
...
assert.ok(!profileEditSource.includes('HeroCertBadge'), '编辑资料头像右侧按最新蓝湖稿只显示昵称，不应叠加认证勾')
assert.ok(profileEditSource.includes('id="profile-edit-avatar"'), '编辑资料圆头像缺少稳定验收节点')
assert.ok(profileEditSource.includes("mode=\"aspectFill\""), '编辑资料圆头像必须保持比例居中裁切')
assert.ok(!profileEditSource.includes('97年丨杭州丨双鱼座'), '编辑资料头像右侧禁止保留演示年龄城市星座')
...
assert.ok(profileEditSource.includes('miniappOssIcons.profileEditCertAvatar'), '头像认证必须使用编辑资料蓝湖灰色线框切图')
assert.ok(profileEditSource.includes('miniappOssIcons.profileEditCertRealName'), '实名认证必须使用编辑资料蓝湖灰色线框切图')
assert.ok(profileEditSource.includes('miniappOssIcons.profileEditCertEducation'), '学历认证必须使用编辑资料蓝湖灰色线框切图')
...
assert.match(profileEditSource, /data-role="about-detail-list"[\s\S]{0,100}marginTop: '28rpx'/, '关于我标题到首个内容必须按蓝湖保留 28rpx 间距')
