...
assert.match(preview, /\{model\.relationshipStatus\}/, '头像区副文案必须由感情状态字段驱动')
assert.doesNotMatch(preview, /model\.datingGoal \|\| model\.relationshipStatus/, '脱单目标不得回填到感情状态展示位')
assert.match(preview, /data-role="profile-preview-basic-divider"/, '基础资料与脱单目标之间必须保留蓝湖分隔线')
assert.match(preview, /data-role="profile-preview-dating-goal"/, '基础资料卡必须提供独立脱单目标展示位')
assert.match(preview, /脱单目标：\{model\.datingGoal \|\| '待完善'\}/, '脱单目标展示位必须绑定真实资料模型')
...
assert.match(visibility, /photos: input\.photos\.map\(normalizeText\)\.filter\(Boolean\)/, '主页预览必须展示全部真实相册图片')
assert.doesNotMatch(visibility, /photos:[^\n]*slice\(/, '主页预览不得截断已上传的真实相册图片')
