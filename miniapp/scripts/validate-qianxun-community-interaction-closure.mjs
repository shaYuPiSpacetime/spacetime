...
assert.match(zhiyin, /data-role="sincere-comment-icon"/, '诚意贴评论必须使用结构化图标')
assert.match(zhiyin, /data-role="sincere-like-icon"/, '诚意贴心动必须使用结构化图标')
...
assert.match(zhiyin, /sincere-\$\{kind\}-stat/, '诚意贴图标必须挂载在固定高度且垂直居中的操作区')
