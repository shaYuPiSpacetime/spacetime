...
assert.match(compose, /data-role=\{`compose-tool-\$\{kind\}`\}/, '发布动态工具栏必须为三类图标输出稳定语义标识')
assert.match(compose, /qianxunComposePhoto/, '发布动态必须使用蓝湖照片工具图标')
assert.match(compose, /qianxunComposeVideo/, '发布动态必须使用蓝湖视频工具图标')
assert.match(compose, /qianxunComposeSmile/, '发布动态必须使用蓝湖表情工具图标')
