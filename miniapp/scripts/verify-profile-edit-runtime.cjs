...
;(async () => {
  ...
  await waitForElementText(page, '#voice-duration', /^10S$/, '语音录制动态读秒与最小时长', 14000)
  await waitForElementText(page, '#voice-recording-limit', /录音时长\s*10-60\s*秒/, '语音录制动态时限')
  ...
  assert.ok(cancelDelete, '删除确认弹层缺少蓝湖左侧取消按钮')
  await cancelDelete.tap()
  await page.waitFor(300)
  assert.equal(await page.$('#voice-confirm-left'), null, '取消删除后确认弹层未关闭')
  await waitForPageText(page, '录制完成', '取消删除后的录音恢复')
  const reopenDelete = await findTextElement(page, '删除')
  assert.ok(reopenDelete, '取消删除后缺少再次删除入口')
  await reopenDelete.tap()
  await waitForPageText(page, '删除提示', '再次打开本地录音删除确认态')
  const confirmDelete = await page.$('#voice-confirm-right')
  assert.ok(confirmDelete, '删除确认弹层缺少蓝湖确认按钮')
  await confirmDelete.tap()
  await page.waitFor(500)
  assert.equal(await page.$('#voice-confirm-left'), null, '删除本地录音后弹层未关闭')
  const restoredVoiceTexts = (await allTexts(page)).join('|')
  assert.ok(
    restoredVoiceTexts.includes('使用语音介绍特别的你') || restoredVoiceTexts.includes('录制完成'),
    '删除本地草稿后必须恢复初始录音态或既有已保存语音态',
  )
