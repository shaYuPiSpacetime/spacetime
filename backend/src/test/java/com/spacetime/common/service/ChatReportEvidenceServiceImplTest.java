...
class ChatReportEvidenceServiceImplTest {
    ...
    void setUp() {
        service = new ChatReportEvidenceServiceImpl(
                recordDao, evidenceDao, new ObjectMapper());
        now = LocalDateTime.of(2026, 8, 10, 12, 0);
    }

    @Test
    @DisplayName("正文存在时复制为案件受控明文快照且不改消息主表")
    void freezeShouldPersistIndependentEvidence() {
        ...
        assertThat(captor.getValue().getContentText()).isEqualTo("原始举报正文");
