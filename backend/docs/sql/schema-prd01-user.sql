...
-- =============================================
-- 注意：本文件保留旧版初始 DDL。2026-07-07 用户已确认后的新增字段、新表、索引与权限，
-- 以 docs/sql/schema-prd01-user.sql 中的“本轮确认增量”和 deploy/sql/prod/011_prd01_user_access_confirmed_delta.sql 为准。
...
CREATE TABLE IF NOT EXISTS app_user (
    ...
    identity VARCHAR(30) DEFAULT NULL COMMENT '身份字典code：app_identity',
    ...
    marital_status VARCHAR(30) DEFAULT NULL COMMENT '婚姻状况字典code：app_marital_status',
...
CREATE TABLE IF NOT EXISTS app_user_verification (
    ...
    profile_photo_audit_source VARCHAR(20) DEFAULT 'MACHINE' COMMENT '资料照片审核来源：MACHINE/MANUAL',
    -- 开放性文字审核
    ...
    open_text_audit_source VARCHAR(20) DEFAULT 'MACHINE' COMMENT '开放性文字审核来源：MACHINE/MANUAL',
    -- 汇总
