...
包体积、分包、切图压缩和微信上传检查门禁以 `TEAM_STANDARDS.md` 的 `10.1.2 小程序上传检查与包体积规范`、`docs/移动端文档/miniapp-lanhu-ui-regression-guard.md` 的 `包体积与上传检查` 为准；流程标为“可通”前必须同时满足主包 `<1.5M`、主包图片/音频 `<200K`、全上传包图片/音频 `<200K`、单资源 `<200K`。蓝湖切图落地时必须自动转 WebP，原始 PNG/JPG 只归档到 `.lanhu-ref`，不得把新大图直接放进 `miniapp/src/assets`。
