...
```text
frontend/src/pages/customers/CustomersPage.tsx  用户管理（增加认证/准入筛选）
frontend/src/pages/verify/VerificationManagementPage.tsx  认证审核（实名/学历/头像 三个 Tab）
frontend/src/pages/moderation/ModerationPage.tsx  资料内容审核（照片/文字 两个 Tab）
  → frontend/src/api/userApp.ts, verification.ts
  → /admin/users/app/**, /admin/verify/**, /admin/moderation/**
...
PRD 要求详情页展示"历次审核状态变更记录（操作人、时间、结果、驳回原因）"。

**v1 策略：** `app_user_verification` 每个认证/审核类型在单次提交→审核流程中只产生一条记录，当前状态字段（`submit_time`、`result_time`、`reject_reason`）本身就是该类型的审核历史。后端详情接口返回当前记录作为"最近一条审核记录"。

**v2 扩展：** 当支持"驳回后重新提交 → 再次审核"的多轮流转时，新增 `app_verification_audit_log` 表：

```sql
CREATE TABLE app_verification_audit_log (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    verification_id BIGINT NOT NULL COMMENT '关联 app_user_verification.id',
    audit_type VARCHAR(30) NOT NULL COMMENT '审核类型: REAL_NAME/EDUCATION/AVATAR/PROFILE_PHOTO/OPEN_TEXT',
    from_status VARCHAR(20) COMMENT '变更前状态',
    to_status VARCHAR(20) NOT NULL COMMENT '变更后状态',
    operator_id BIGINT COMMENT '操作人ID（系统自动=null）',
    operator_name VARCHAR(50) COMMENT '操作人姓名',
    reject_reason VARCHAR(200) COMMENT '驳回原因',
    create_time DATETIME DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_verification_id (verification_id),
    INDEX idx_audit_type (audit_type)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='认证审核历史日志（v2启用）';
```

v1 建表但不写入，v2 在每次审核操作时插入日志并关联到详情接口返回。
...
| Admin VO | `admin/dto/response/AppUserDetailVO.java` | 用户详情出参（含内嵌 VerificationBlock） |
| Admin VO | `admin/dto/response/AppUserListVO.java` | 用户列表出参 |
| Admin VO | `admin/dto/response/VerificationVO.java` | 认证审核列表出参 |
| Admin VO | `admin/dto/response/VerificationAuditDetailVO.java` | 认证审核详情出参（含 FieldEntry 内嵌类） |
| Admin VO | `admin/dto/response/ModerationVO.java` | 内容审核列表出参 |
| Admin VO | `admin/dto/response/ModerationDetailVO.java` | 内容审核详情出参 |
...
| 用户详情 | `/admin/users/app/{id}` | GET | `user:app:detail` | `id` | `AppUserDetailVO` |
...
| 实名认证审核 | `/admin/verify/real-name/{id}/audit` | POST | `verify:realname:audit` | `ModerationAuditReq` | `Void` |
| 学历认证审核列表 | `/admin/verify/education/list` | GET | `verify:education:list` | `VerificationPageReq` | `Page<VerificationVO>` |
| 学历认证审核 | `/admin/verify/education/{id}/audit` | POST | `verify:education:audit` | `ModerationAuditReq` | `Void` |
| 头像认证审核列表 | `/admin/verify/avatar/list` | GET | `verify:avatar:list` | `VerificationPageReq` | `Page<VerificationVO>` |
| 头像认证审核 | `/admin/verify/avatar/{id}/audit` | POST | `verify:avatar:audit` | `ModerationAuditReq` | `Void` |
| 实名认证详情 | `/admin/verify/real-name/{id}` | GET | `verify:realname:list` | `id` | `VerificationAuditDetailVO` |
| 学历认证详情 | `/admin/verify/education/{id}` | GET | `verify:education:list` | `id` | `VerificationAuditDetailVO` |
| 头像认证详情 | `/admin/verify/avatar/{id}` | GET | `verify:avatar:list` | `id` | `VerificationAuditDetailVO` |
...
| 照片审核 | `/admin/moderation/photos/{id}/audit` | POST | `moderation:photo:audit` | `ModerationAuditReq` | `Void` |
| 文字审核列表 | `/admin/moderation/texts/list` | GET | `moderation:text:list` | `VerificationPageReq` | `Page<ModerationVO>` |
| 文字审核 | `/admin/moderation/texts/{id}/audit` | POST | `moderation:text:audit` | `ModerationAuditReq` | `Void` |
| 照片审核详情 | `/admin/moderation/photos/{id}` | GET | `moderation:photo:list` | `id` | `ModerationDetailVO` |
| 文字审核详情 | `/admin/moderation/texts/{id}` | GET | `moderation:text:list` | `id` | `ModerationDetailVO` |
...
| `AppUserPageReq` | extends PageReq | + keyword/nickname/school/accountStatus/gender/realNameStatus/educationStatus/avatarVerifyStatus/firstLoginCompleted/userId/registerTimeStart/registerTimeEnd | 多条件筛选；userId精确匹配，registerTime范围查询 |
| `AppUserListVO` | id/avatar/nickname/gender/age/school/realNameStatus/educationStatus/avatarVerifyStatus/firstLoginCompleted/profileScore/accountStatus/registerTime/lastLoginTime/accessStatus | 列表行；accessStatus为计算字段（见§7.7）；手机号需微信授权v2补 |
| `AppUserDetailVO` | (VerificationBlock)/canBrowseCards/canMatch/canBeExposed/blockReason/violationCount/feedbackCount | 四区块：基本资料+认证信息(内嵌VerificationBlock)+准入信息(AccessStatus计算)+关联信息(v1预留) |
| `VerificationAuditDetailVO` | id/userId/nickname/avatar/verifyLevel/fields(List\<FieldEntry\>)/submitTime/resultTime/rejectReason/status | 泛化结构承载三类认证内容差异，FieldEntry={label,value}：实名→[姓名,身份证号,核身结果]；学历→[学校,认证方式,材料摘要,第三方结果]；头像→[当前头像URL,历史认证记录] |
| `ModerationDetailVO` | id/userId/nickname/avatar/contentType/contentFull/contentField/submitTime/status/rejectReason | 内容审核详情；照片类型contentFull=原图URL，文字类型contentFull=完整文本+contentField=字段类型 |
...
认证审核和内容审核采用**单组件多路由**模式：一个组件通过 `useLocation()` 感知当前路径，内部 Tab 切换子页面，避免重复代码。

| 页面 | 路由 | 组件 | 权限 |
| --- | --- | --- | --- |
| 用户管理（扩展） | `/customers` | `pages/customers/CustomersPage.tsx` | `user:app:list` |
| 实名认证审核 | `/verify/real-name` | `pages/verify/VerificationManagementPage.tsx` | `verify:realname:list` |
| 学历认证审核 | `/verify/education` | `pages/verify/VerificationManagementPage.tsx` | `verify:education:list` |
| 头像认证审核 | `/verify/avatar` | `pages/verify/VerificationManagementPage.tsx` | `verify:avatar:list` |
| 资料照片审核 | `/moderation/photos` | `pages/moderation/ModerationPage.tsx` | `moderation:photo:list` |
| 文字内容审核 | `/moderation/texts` | `pages/moderation/ModerationPage.tsx` | `moderation:text:list` |

路由更新 `frontend/src/router/index.tsx`，新增 5 个路由指向 2 个组件。`VerificationManagementPage` 内部根据 `location.pathname` 激活对应 Tab（实名/学历/头像），`ModerationPage` 同理（照片/文字）。参考 `CommunityManagementPage` 的 Tab 切换模式。
...
**列表视图：**
- 列表展示：用户头像+昵称、认证状态(Badge)、提交时间、驳回原因(截断)
- 操作：通过/驳回（仅 PENDING 状态显示，带驳回原因输入）
- 筛选：昵称模糊搜索、状态（全部/待审核/已通过/已驳回）

**点击行 → 审核详情 Dialog（Modal）：**

实名认证详情：
- 用户信息：头像、昵称、用户ID
- 认证内容：真实姓名(脱敏) / 身份证号(脱敏) — `user:app:detail` 权限可查看完整
- 微信核身结果摘要
- 提交时间
- 审核历史列表：每次状态变更的操作人、时间、结果、驳回原因
- 操作按钮：通过 / 驳回（必填原因）/ 标记跟进

学历认证详情：
- 用户信息：头像、昵称、用户ID
- 认证内容：学校、认证方式(CHSI/ONLINE_CODE/DIPLOMA_NO)、认证材料摘要
- 第三方认证服务商返回结果（如有）
- 提交时间
- 审核历史列表
- 操作按钮：同上
...
头像认证详情：
- 用户信息：昵称、用户ID
- 当前主头像（大图 >=300px，点击放大至原图）
- 历史头像认证记录（缩略图 + 审核结果 + 时间）
- 审核标准提示文案
- 审核历史列表
- 操作按钮：同上
...
**列表视图：**
- 照片审核列表：用户头像+昵称、内容类型、内容预览(照片URL渲染为缩略图)、状态(Badge)、提交时间、驳回原因
- 文字审核列表：用户头像+昵称、内容类型(字段类型)、文本摘要(截断50字)、状态(Badge)、提交时间、驳回原因
- 操作：通过/驳回（仅 PENDING 状态显示）
- 筛选：昵称模糊搜索、状态（全部/待审核/已通过/已驳回）
...
- `frontend/src/api/verification.ts` — 后台认证审核+内容审核接口（统一用 `keyword` 昵称模糊搜索替代 `userId` 入参）

### 8.4 前端枚举中文化映射

后端返回的枚举值为英文 code（如 `BACHELOR`、`CHSI`、`LOOKING`），前端通过映射表转为中文展示：

| 映射表 | 所在文件 | 示例 |
| --- | --- | --- |
| `STATUS_MAP` | CustomersPage / VerificationPage | `PENDING→待审核`, `EXPIRED→已失效` |
| `EDUCATION_LEVEL_MAP` | CustomersPage | `BACHELOR→本科`, `MASTER→硕士`, `DOCTOR→博士` |
| `EMOTIONAL_STATUS_MAP` | CustomersPage | `LOOKING→单身中`, `NOT_LOOKING→暂不找`, `SECRET→保密` |
| `DATING_GOAL_MAP` | CustomersPage | `SERIOUS_RELATIONSHIP→认真恋爱`, `CASUAL→随缘` |
| `MARITAL_STATUS_MAP` | CustomersPage | `UNMARRIED→未婚`, `MARRIED→已婚`, `DIVORCED→离异` |
| `EDUCATION_METHOD_MAP` | VerificationPage | `CHSI→学信网`, `ONLINE_CODE→在线验证码`, `DIPLOMA_NO→学历证书编号` |
| `CONTENT_TYPE_MAP` | ModerationPage | 防御性映射，确保后端返回变更时前端有兜底 |
| `formatFieldValue()` | VerificationPage | 根据 FieldEntry.label 自动选择映射（认证方式/人脸核身状态/认证状态） |

所有映射表均有兜底：未匹配值直接展示原始值或 `-`。

## 9. 权限与菜单

### 9.1 菜单树

所有 PRD-01 相关菜单统一挂靠在"用户准入"目录下：

| 菜单 | 类型 | 路由 | 组件 | 权限 |
| --- | --- | --- | --- | --- |
| 用户准入 | M | - | - | - |
| App用户管理 | C | `/customers` | `users/AppUserManagement` | `user:app:list` |
...
| `user:app:gender:edit` | 修改用户性别（客服操作，需记录日志） |
...
| Admin VO | `admin/dto/response/AppUserDetailVO.java` | 新增 | 含内嵌 VerificationBlock |
| Admin VO | `admin/dto/response/AppUserListVO.java` | 新增 | - |
| Admin VO | `admin/dto/response/VerificationVO.java` | 新增 | - |
| Admin VO | `admin/dto/response/VerificationAuditDetailVO.java` | 新增 | 认证审核详情（含 FieldEntry） |
| Admin VO | `admin/dto/response/ModerationVO.java` | 新增 | - |
| Admin VO | `admin/dto/response/ModerationDetailVO.java` | 新增 | 内容审核详情 |
| SQL | `backend/docs/sql/schema-prd01-audit-log.sql` | 新增 | v2 审核历史日志表 DDL（v1 建表不写入） |
