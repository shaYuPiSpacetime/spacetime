...
> 需求编号：PRD-03  
> 流程阶段：阶段 3 - 技术方案  
> 当前状态：2026-08-14 已完成后端代码、管理后台消息互动页面、真实数据库流程数据和自动化验证：14 表、正文留存、TIM 单一映射、私信已读、四类未读汇总、四列表固定分页、受控高敏查看及审计均已落地；消息模块已移除应用层 KMS，生产开放仍受腾讯云 TIM 和法务复核门禁约束
> 采用技能：`techni-design`
...
> **2026-08-14 存储口径确认**：系统消息、官方助手、消息事件临时载荷和举报冻结证据不再使用应用层 KMS/AES。平台数据按受控明文落库；普通接口、列表、导出和日志不返回正文，举报证据仍要求有效案件权限、查看原因、二次确认和访问审计。数据库磁盘与备份加密由基础设施层独立承接，不作为消息接口可用性门禁。
...
| TD03-11 | 系统/助手消息、Inbox 临时载荷和举报冻结证据使用受控明文；不注入应用层 Cipher，不因 KMS 配置缺失阻断消息首页 |
...
| 历史实现曾把 KMS 作为系统消息和举报证据门禁 | 旧 Cipher 实现与密文字段 | 已移除应用层 Cipher；系统/助手、临时事件载荷和举报证据统一按受控明文方案处理 |
...
`admin/` 与 `miniapp/` 不互相 import。状态机、存储安全、Outbox、案件证据和跨模块适配放在
...
| common Service | `MessageSensitiveContentService/Impl` | 仅按权限读取 PRD-05 已冻结案件证据，执行复合授权与审计前置 |
...
- 基线迁移脚本：`deploy/sql/prod/070_prd03_message_center_closure.sql`；既有库移除 KMS 增量：`deploy/sql/prod/074_prd03_remove_message_kms.sql`。
- 字符集统一 `utf8mb4`，引擎 `InnoDB`，时间使用 `DATETIME`。
- 所有业务号由服务端生成，接口不暴露 BIGINT 主键。
- `app_message_record.content_text` 按产品确认使用明文 `TEXT`。业务消息创建时必须写入，终态隔离满 180 天后仅将该列置空并记录 `content_cleared_at`，消息编号、参与方、TIM 映射和发送事实继续保留；不得建立正文索引、进入普通查询/导出/日志。系统消息、官方助手使用 `title_text/content_text`，举报冻结证据使用受控 `content_text` 明文。
- `app_message_event_inbox.payload_json` 只保存模板变量等有界临时数据，禁止聊天正文；成功、死信或到期后清空。旧密文字段仅为非破坏升级保留，应用停止读写。
...
| `title_text` | VARCHAR(200) | 否 | NULL | 标题明文；新消息写入 |
| `content_text` | TEXT | 否 | NULL | 正文明文；新消息写入 |
...
| `payload_json` | MEDIUMTEXT | 否 | NULL | 处理前有界临时业务 JSON；不得包含聊天正文，处理结束后清空 |
...
Inbox 是可靠消费记录，不是第二份业务事实表。聊天正文只能存在消息主表；仅系统消息渲染变量等
非正文控制数据允许写入有界临时 `payload_json`。消费成功必须在同一事务清空 `payload_json` 并写
`payload_cleared_at`；失败事件保留到
...
访问尝试，成功后更新结果；即使正文读取或响应失败，也保留尝试记录。
...
| `content_text` | MEDIUMTEXT | 是 | 无 | 举报冻结时保存的受控明文正文 |
...
| `caseContentConfirmModal/punishModal` | 冻结正文/处罚 | 案件内弹层 | 案件号/证据号/原因 | 查看/处罚确认 | Modal | 越权/证据不属/审计失败 | textarea/危险按钮 | 单条正文 | 不适用 | no-store/失败提示 | 案件 -> 审计 -> 受控读取 | 截图+审计 |
...
系统消息、官方助手、消息事件临时载荷和举报冻结证据均不使用应用层 KMS/AES。系统消息与
官方助手写入 `title_text/content_text`；举报冻结证据写入 `community_report_evidence.content_text`。

| 项 | 设计 |
|----|------|
| 存储 | 数据库受控明文；不再保存应用层密文、IV、keyVersion 或 HMAC |
| 普通访问 | 通用列表、统计、搜索、导出和日志字段白名单排除正文 |
| 案件访问 | 仅有效举报案件处理人按单条查看；要求权限、案件归属、查看原因、二次确认和审计 |
| 事件载荷 | Inbox 只保存模板变量等有界 `payload_json`，禁止聊天正文；成功、死信或到期后清空 |
| 历史兼容 | 升级脚本保留旧密文字段但停止读写；无法恢复的旧密文载荷转死信并要求上游重投 |
| 基础设施 | 磁盘、快照和备份加密由数据库/云基础设施独立配置，不进入应用业务流程 |
| 日志 | 禁止记录正文、UserSig、SecretKey、发送许可和完整事件 payload |
...
5. 任一业务事实不可用时拒绝发送，不调用支付或其他非必要外部 RPC。
...
| Inbox 临时业务 JSON | 成功消费前或失败重试窗口内 | 成功事务立即清空；`dead` 或 `payload_expires_at` 到期清空，Inbox 幂等和处理结果继续保留 |
...
| 普通系统消息 | 用户侧 730 天 | 清空标题/正文并逻辑删除，保留必要业务元数据 |
...
| Provider 合同 | IM REST/消息前后回调、UserSig 超时与重试 | MockWebServer/Provider fake |
...
2. 发布消息主表、系统/助手、举报证据受控明文归档、IM Provider、Inbox/Outbox 和只读业务 API，发送入口保持关闭。
3. 配置腾讯消息前后回调和监控，完成 Provider 合同测试。
...
| provider | `backend/src/main/java/com/spacetime/common/provider/` | 新增/修改 | IM、UserSig 实现 |
...
| T03-04 | 存储安全 | 平台系统/助手消息、Inbox 临时载荷和独立举报证据使用受控明文，不依赖应用层 KMS | 运维、安全、可用性 | 采用字段白名单、案件权限、查看原因、审计和数据库基础设施权限；KMS 缺失不得阻断消息首页 | **已确认（2026-08-14）** |
...
| T03-09 | 外部门禁 | 腾讯 IM 应用/消息前后回调、7 张移动端 UI、法务隐私复核 | 联调与生产上线 | 方案已确认；资料与环境仍需在对应阶段提供，缺 TIM 回调和法务结论不得开放生产发送 | **外部准备中，不是需求待确认项** |
...
| T03-12 | 14 表结构修正 | 保留 14 张表；正文到期仅清空主表 `content_text` 并保留元数据；TIM 消息映射只存消息主表，悄悄话按消息外键读取；Inbox 载荷为禁止聊天正文的有界临时 JSON，成功、死信或到期必须清空 | 数据库、DAO、可靠任务、留存 | 以单一事实源消除映射漂移，并补齐临时数据的可执行生命周期 | **已确认（2026-08-14 更新）** |
...
准备门禁，不阻塞代码设计与测试文档，但缺少对应真实资源时不得宣称 TIM 联调或生产开放完成。

## 22. 实施计划

| 步骤 | 改动范围 | 核心交付 | 验证方式 |
|------|----------|----------|----------|
| 1 | 外部 POC/配置 | LiteChat 4.4.2、消息前后回调、UserSig、TIM REST 可用性 | 独立合同 POC，不改全页面 |
| 2 | SQL 与实体 | 迁移、实体、枚举、DAO/DAOImpl/Mapper | 迁移重复执行、索引和唯一约束测试 |
| 3 | 安全基础 | IM Provider、消息前回调鉴权、正文列访问白名单、案件权限审计、限流 | Provider 单测、字段白名单、权限与日志扫描 |
...
| 未包含 | 未修改小程序前端；真实 TIM 双账号、隔离 MySQL/Redis 与法务验收仍属于上线门禁 |
...
| 详情 | 右侧抽屉展示元数据；系统消息和官方助手消息直接展示标题/正文，私信和悄悄话只显示“查看高敏内容”入口 |
...
| `system_message` | 详情 Service 读取 `title_text/content_text`，直接在详情响应返回 | 无额外弹窗 | `message:record:list`；普通详情访问日志；响应建议 `private, no-store` |
| `assistant_message` | 详情 Service 读取 `title_text/content_text`，直接在详情响应返回 | 无额外弹窗 | `message:record:list`；普通详情访问日志；响应建议 `private, no-store` |

`AdminMessageRecordDetailVO` 增加 `title/content/contentFormat/actionText` 等平台消息展示字段及 `contentAvailable/sensitiveContent` 标志。私信、悄悄话分支必须保证这些明文字段为 `null`；系统/助手直接返回平台明文字段。
...
聊天类详情新增“聊天举报上下文”和“冻结证据”区块，展示来源场景、业务编号、会话编号、发送/接收方脱敏标识、消息类型、事件时间、证据顺序、快照时间、留存截止时间和正文可用状态。正文查看需同时满足：
...
5. 证据不存在、已过留存期、无权限、案件终止或正文不可用时只显示明确错误，不回退读取消息主表正文。
