...
| 会话 | `app_message_conversation_member` | 新增 | 参与者与对方映射；不保存普通私信已读游标或未读 |
...
| `tim_conversation_id` | VARCHAR(128) | 是 | 无 | 腾讯云 TIM C2C 会话标识 |
...
| 阶段 | 权威状态 | 平台动作 |
|------|----------|----------|
| SDK 创建/发送 | LiteChat `sending` | 小程序保留原消息对象和随机号，平台不创建发送 Outbox |
| 消息前回调允许 | TIM 待投递 | 平台只做最终业务授权；正文和已读初态由发送后回调归档 |
| TIM 发送成功 | LiteChat `sent` | 消息后回调按 `SDKAppID + MsgKey` 幂等写 `app_message_record.content_text`、TIM 映射、`receiver_read_status=unread` 和业务活动时间 |
| TIM 发送失败 | LiteChat `failed` | 小程序按 SDK 契约重试；平台不伪造成功或新建第二条消息 |
...
必须进入回调重试/对账告警，不能把该消息标记为平台归档完成。小程序展示会话后通过专用 read 接口
按 `lastMessageNo` 幂等推进平台已读状态。
