...
## 19. 2026-08-21 小程序体验版固定验证码增量用例

> 测试模式：增量模式；后端 Service 核心登录逻辑变更，执行 L3 JUnit 单元测试。

| 用例 ID | 层级 | 优先级 | 场景 | 断言 |
|---------|------|--------|------|------|
| L3-MINI-AUTH-001 | L3 | P0 | 请求发送手机验证码 | Redis 写入固定验证码 `0000`，返回 `providerCode=FIXED`，不调用真实短信网关 |
