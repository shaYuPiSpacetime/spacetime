...
| TC-14 | 生产微信预下单 | 为具有真实 openid 的生产用户创建短时测试会话，调用 `POST /miniapp/payment/create-order` | HTTP/业务码均为 200，返回 `package=prepay_id=...`、RSA `paySign`，订单落库并保存预支付标识；测试会话自动删除且不触发扣款 |
