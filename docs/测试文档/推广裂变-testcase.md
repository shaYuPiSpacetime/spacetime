...
| 渠道运营 Token | 代理与代理码管理 | 创建具备 `promotion:agent:*` 的角色 | 是 |
...
| F1-P0-04 | P0 | 记录普通分享来源 | `POST /miniapp/promotion/invite/share-log` | 邀请人存在 | 自构建邀请人 | 返回 `traceNo`，来源状态 `unbound` | 重新查询验证状态 |
| F1-P0-05 | P0 | 记录代理扫码来源 | `POST /miniapp/promotion/invite/share-log` | 代理码启用 | 自构建代理和代理码 | 返回 `traceNo`，写入代理 click 事件 | 重新查询验证状态 |
| F1-P0-06 | P0 | 新用户绑定普通邀请关系 | `POST /miniapp/promotion/invite/bind` | 已有普通来源 trace，新用户首次登录 | 链式 | 建立关系，状态 `login_success` | 查询邀请详情/记录 |
| F1-P0-07 | P0 | 新用户绑定代理关系 | `POST /miniapp/promotion/invite/bind` | 已有代理来源 trace，新用户首次登录 | 链式 | 建立代理归属，不生成普通用户奖励 | 查询代理事件 |
| F1-P1-01 | P1 | 查询代理来源 | `GET /miniapp/promotion/invite/agent-source` | 代理码启用 | 自构建代理码 | 返回代理来源可用、小程序路径信息 | 响应断言 |
| F1-P1-02 | P1 | 获取邀请海报 | `GET /miniapp/promotion/invite/poster` | 邀请人已登录 | 小程序 Token | 返回海报/二维码 URL 或可生成状态 | 响应断言 |
| F1-P1-03 | P1 | 邀请记录按状态筛选 | `GET /miniapp/promotion/invite/records?status=frozen` | 存在冻结记录 | fixture | 只返回冻结记录 | 响应断言 |
| F1-P2-01 | P2 | 分享来源缺少来源类型 | `POST /miniapp/promotion/invite/share-log` | 无 | 缺参请求 | 返回参数错误 | 响应断言 |
| F1-P2-02 | P2 | 无效代理码查询 | `GET /miniapp/promotion/invite/agent-source` | 代理码不存在或停用 | 固定值 | 返回不可用或业务错误 | 响应断言 |
...
| F1-P2-06 | P2 | 同时携带普通邀请码和代理码 | `POST /miniapp/promotion/invite/bind` | 普通邀请码和代理码均有效 | 自构建 | 归属代理，普通邀请不发币 | 查询关系和奖励流水 |
...
| F3-P0-06 | P0 | 生成代理码 | `POST /admin/promotion/agents/{id}/codes/regenerate` | 代理存在 | 链式 | 返回唯一 `agentCode`、小程序路径 | 查询代理详情 |
...
| F3-P1-01 | P1 | 停用代理码 | `PUT /admin/promotion/agent-codes/{id}/disable` | 代理码 enabled | 链式 | 状态 disabled，后续不建立新代理关系 | 查询代理码/绑定验证 |
...
| L2-09 | `PromotionAgentControllerTest.regenerateCode_shouldReturnAgentCodeVO` | 代理码生成接口路由 | 返回 `agentCode/miniappPath` |
...
| L2-14 | `PromotionInviteControllerTest.bind_shouldValidateSource` | 绑定接口 trace/inviteCode/agentCode 至少一个有效 | 全空返回参数错误 |

## 5. L3 - Service 单元测试用例

| 用例ID | 测试方法 | 输入 | 期望输出 |
|--------|----------|------|----------|
| L3-01 | `PromotionInviteServiceTest.bind_normalInvite_shouldCreateRelation` | 新用户、普通 trace、有效 inviter | 创建 `promotion_invite_relation`，状态 `login_success` |
...
| L3-05 | `PromotionInviteServiceTest.bind_agentPriority_shouldPreferAgent` | 同时携带普通邀请码和代理码 | 关系归属代理，普通邀请不发币 |
...
| L3-14 | `PromotionAgentServiceTest.createAgent_shouldGenerateUniqueCode` | 新增代理并生成码 | agentCode 唯一，状态 enabled |
| L3-15 | `PromotionAgentServiceTest.disabledAgentCode_shouldRejectNewRelation` | 停用代理码后绑定 | 不建立代理关系 |
...
| L4-05 | P0 | 代理列表 | 渠道运营登录 → 新增代理 → 生成代理码 | 列表出现代理，详情展示 agentCode 和小程序路径 |
| L4-06 | P0 | 代理结算管理 | 财务登录 → 选择代理和周期 → 生成结算单 → 标记确认 → 标记已发放 | 结算状态依次变为 `pending/confirmed/paid` |
| L4-07 | P1 | 推广素材与二维码 | 渠道运营进入素材页 → 下载/停用代理码 | 代理码状态变化，按钮权限正确 |
