...
|---------|--------|------|----------|----------|
| L4-01 | P0 | `/commercial/config` | 打开千寻币消费场景 Tab，内联编辑一行 | 表格完整回显后台固定场景名、移动端展示名称、图标、说明、价格、启停和影响页面 |
...
| L4-08 | P0 | `/commercial/config` | 逐一切换 7 个配置 Tab，对照静态 Demo 核对表头、固定后台名称、字段说明和只读项 | 会员权益、会员套餐、千寻币套餐、消费场景、保留期、社交与订单参数、曝光包预留均使用同一列与字段契约；动态值仍取接口数据 |
...
| M-08 | P0 | 打开更新后的静态 Demo 与本地正式 `/commercial/config`，逐 Tab 横向核对 | 7 个 Tab 的列名、固定名称、说明文案和只读项一致；会员套餐只保留普通套餐与一次性购买 | Playwright 逐 Tab 截图与 DOM 契约核对通过 | PASS |
