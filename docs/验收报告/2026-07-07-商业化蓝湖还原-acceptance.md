...
- 千寻币用途与会员权益图标已从散落字符/结构化占位升级为真实图片组件：千寻币用途已拆为 8 个独立 2x PNG 并原样上传上海 2 OSS，会员权益使用 08 页面导出的独立图标切片并统一引用 OSS 清单。
...
- 千寻币首页通过蓝湖 MCP `slices` 获取 `金币` 与 `千寻币用途` 两个切片：`金币` 已用于套餐数量前缀；千寻币用途已拆为 8 个独立 2x PNG，运行时使用 OSS 原样 URL 与真实文字组件，避免整卡文字和图标重影。
...
- 当前项目可复用商业化资产包括 `coin-balance-bg.webp`、`coin-gold.png`、`coin-usage/*.png`、`member-benefits/*` 以及我的页部分卡片图；非底部静态图标和我的页静态按钮/横幅已按源文件字节上传上海 2 OSS。`member-vip-bg.webp` 因含旧文案和头像残影，已不再作为会员卡背景使用。
