...
| 版本 | 日期 | 修改人 | 变更摘要 |
|------|------|--------|----------|
| 版本02 | 2026-07-02 | Codex | 按第 1 轮 Claude 核查整改移动端展示附加属性 |
...
|------|------|------|------------|------------------|
| 顶部标题栏 | 顶部 | 标题、通知图标、通知未读红点 | 否 | 否 |
| 官方消息卡片 | 标题下方 | 官方头像、名称、最新摘要、时间、未读红点 | 否 | 否 |
...
| 画板 ID | 画板名称 | 设计内容 | 备注 |
|---------|----------|----------|------|
| `APP-03-msg-list-01` | 消息列表页-主页面 | 官方消息卡片、私信列表、红点 | P0 |
| `APP-03-msg-list-02` | 消息列表页-未认证空态 | 官方消息保留，私信区认证引导 | P0 |
...
|---------|--------|------|------|----------|----------|--------|--------|----------|----------|
| `APP-03-PAGE-message-list-FIELD-official-title` | 官方消息标题 | string | 是 | 1-20 字 | 服务端返回非空 | 官方消息 | 后台模板配置 | 普通 | `M03-CFG-notification-template-list` |
| `APP-03-PAGE-message-list-FIELD-official-preview` | 官方消息摘要 | string | 否 | 0-50 字 | 超长省略 | 无 | 系统生成 | 普通 | 最新官方通知 |
| `APP-03-PAGE-message-list-FIELD-official-time` | 官方消息时间 | datetime | 否 | datetime | 展示相对时间 | 无 | 不可编辑 | 普通 | 最新官方通知 |
...
|---------|--------|----------|----------|----------|--------|--------|----------------|
| `APP-03-PAGE-message-list-ACT-open-official` | 打开官方消息 | 官方消息卡片可见 | `GLB-ROLE-app-user` | 否 | 进入 `APP-03-PAGE-notification-center` | toast + `M03-ERR-notification-not-found` | 官方未读可在详情中置已读 |
...
Given 用户已登录但未完成三重认证
When  用户点击底部 Tab 消息
Then  页面展示官方消息卡片，不展示用户私信会话，并展示 `M03-TXT-core-access-chat-block` 与去认证入口
```
