...
| 版本 | 日期 | 修改人 | 变更摘要 |
|------|------|--------|----------|
| 正式版02 | 2026-07-16 | Codex | 明确隐藏访问一期完全不开发，隐私设置无入口、开关或会员权益校验 |
...
| 编号 | 来源 | 结论 | 文档落点 |
|------|------|------|----------|
| M06-01 | 找茬 M06-01、一期目标 | 推荐给好友入口纳入一期，由 PRD-07 定义规则；每日签到保留为福利中心单页入口，由 PRD-10 定义规则；活动类历史入口隐藏。PRD-06 只聚合入口。 | `M06-RULE-my-entry-scope`、APP-06 我的页 |
...
| `M06-RULE-my-entry-scope` | 我的页入口聚合 | APP-06-PAGE-my | 我的页可展示 PRD-01 资料/认证、PRD-04 会员/千寻币、PRD-07 推荐给好友、PRD-10 每日签到、我的动态、帮助客服、设置入口；具体规则引用对应模块 | PRD-06 不复述资产、邀请、签到、社区规则 |
| `M06-RULE-cert-center` | 认证中心承接 | APP-06-PAGE-cert-center | 认证为异步流程，不建设独立认证成功页；提交后返回认证中心，由认证卡片展示未认证、审核中、已认证或驳回状态，首次提交、重提、审核流引用 PRD-01 | 不新增婚况认证 |
...
| `M06-RULE-privacy-settings-scope` | 隐私设置范围 | APP-06-PAGE-privacy | 一期隐私设置只承接账号注销入口和隐私说明；黑名单、动态屏蔽、关键词屏蔽、算法授权开关、隐藏访问入口或开关均不作为独立设置项。隐藏访问一期不实现会员权益校验、访客过滤、统计排除和后台筛选 | 场景内安全动作引用 PRD-03/05；隐藏访问仅作后续预留 |
...
| `M06-RULE-scene-safety-handoff` | 场景内安全闭环 | APP/ADM | 用户主页、社区详情、会话页、推荐卡等场景可提供举报、拉黑、屏蔽或不再推荐；具体入口与处罚由 PRD-03、PRD-05 及推荐模块负责人承接，PRD-06 不提供集中管理列表 | 满足一期最小安全闭环 |
| `M06-RULE-safety-content-scope` | 安全提示内容范围 | APP/ADM | 移动端不开放安全中心独立页；一期安全提示、反诈提醒和平台规范入口及文案由代码固定，不建设独立后台配置页 | 不做治理公示统计；内容变更随版本发布 |
| `M06-RULE-search-scope` | 入口限定搜索范围 | APP-06-PAGE-search-home、APP-06-PAGE-search-result | 搜索页根据 `sourceScene` 限定结果 Tab。默认全局入口支持用户/动态/话题；社区入口支持动态/话题；推荐/理想型条件筛选不在 PRD-06 定义，由对应模块负责人另行确定 | 学校、职业、测评报告不纳入 PRD-06 |
...
| AI 个性化推送、配对聊天提示、智能灵感回复开关 | 推荐算法和 AI 辅助策略未定 | 隐藏入口 | 由对应模块负责人另行确定 |
| 搜索热词、搜索展示配置、搜索运营平台和算法排序解释 | 一期只做固定搜索展示和动态屏蔽词 | 删除热词；Tab、空态文案、违规提示、排序写死；后台仅保留屏蔽词 | 推荐算法与筛选策略由其他负责人另行确定 |
