...
| 版本04 | 2026-07-01 | Codex | 明确会员中心首版仅展示时空邂逅会员，不展示 SVIP/高端服务套餐 |
...
- **入口来源**：我的页「时空邂逅会员」；业务场景会员引导弹窗；支付结果页
...
- **目标用户**：已登录用户
- **核心任务**：查看时空邂逅会员状态、理解会员权益，在横向套餐列表中优先查看连续订阅套餐，并可右滑选择普通套餐完成开通/续费；首版不展示 SVIP/高端服务套餐
- **页面类型**：购买页
...
| 权益列表 | 中部 | `M04-CFG-vip-benefit-list` 返回的 9 项权益，前台名称和移动端图标均按后台配置展示 | 否 | 否 |
| 套餐区 | 中部 | 横向平铺套餐卡片，连续订阅套餐排在前面并可默认选中/强化推荐，普通套餐排在后面右滑可见；不展示 SVIP/高端服务套餐 | 否 | 是 |
...
| `APP-04-PAGE-vip-center-FIELD-benefits` | 权益列表 | json | 是 | `M04-CFG-vip-benefit-list` | 仅展示启用权益；固定顺序展示心动名单、访客、免费悄悄话、额外浏览、高级筛选、曝光、隐私权益、三天回放、每日心动机会；每项必须返回 `mobileIcon` | 空列表 | 否 | 普通 | 后台商业化配置 |
| `APP-04-PAGE-vip-center-FIELD-package-list` | 套餐列表 | json | 是 | `M04-ENUM-vip-package-type` | 合并展示普通套餐与连续订阅套餐；连续订阅套餐排在前，普通套餐排在后，支持横向滑动 | 连续订阅优先 | 否 | 普通 | `M04-CFG-vip-package-list` / `M04-CFG-subscription-package-list` |
| `APP-04-PAGE-vip-center-FIELD-package-card` | 套餐卡片 | json | 是 | 上架套餐 | 价格、有效天数/续费周期、套餐编号不能为空；连续订阅商品不可用时对应卡片置灰，不影响普通套餐选择 | 无 | 用户选择 | 普通 | `M04-CFG-vip-package-list` / `M04-CFG-subscription-package-list` |
...
| 展示顺序 | 权益 code | 前台 UI 名称 | 后台能力名 | 展示值来源 | 移动端图标来源 | 说明 |
|----------|-----------|--------------|------------|------------|----------------|------|
| 1 | `heart_list` | 心动名单一键揭晓 | 心动名单 | 开关 | `M04-CFG-vip-benefit-list.mobileIcon` | 原全量查看拆分项 |
| 2 | `visitor_list` | 谁来看过你 / 访客全公开 | 访客 | 开关 | `M04-CFG-vip-benefit-list.mobileIcon` | 原全量查看拆分项 |
| 3 | `free_whisper` | 每日专属悄悄话 | 免费悄悄话 | 开关 + `M04-CFG-vip-free-whisper-daily` | `M04-CFG-vip-benefit-list.mobileIcon` | 用完后继续走千寻币 |
...
|----------|----------|----------|----------|------|
| 套餐列表 | 加载成功 | 套餐区 | 连续订阅套餐前置展示并可默认选中；普通套餐在同一横向列表后段展示，用户右滑可选择 | `M04-TERM-subscription` |
| 选中套餐 | 选中连续订阅套餐 | 协议勾选 | 必须额外展示连续订阅服务协议；支付成功后进入真实自动续费关系 | `M04-TERM-subscription` |
| 套餐列表 | 出现 SVIP/高端服务 | 套餐区 | 不展示该类型，视为配置不合法并提示刷新套餐 | `M04-RULE-member-tier-scope` |
...
| 会员权益配置 | 移动端图标变更 | 权益列表 | 刷新后按 `mobileIcon` 展示对应图标；名称/说明仍按权益配置与文案展示 | `M04-RULE-vip-benefit-config-fields` |
...
| 异常态 | 返回 SVIP/高端服务套餐 | 不展示该套餐并刷新配置 | 刷新 | `M04-RULE-member-tier-scope` |
| 降级态 | 支付服务不可用 | 支付按钮置灰 | 稍后重试 | `M04-SRV-wechat-pay` |
| 降级态 | 连续订阅服务不可用 | 连续订阅套餐卡片置灰并提示服务不可用；普通套餐仍在横向列表中可选；管理入口展示重试或取消指引 | 选择普通套餐/稍后重试 | `M04-SRV-wechat-subscription` |
...
AC-ID: APP-04-AC-vip-benefit-ui-names
Given 后台会员权益配置启用 9 项权益
When  用户进入时空邂逅会员中心
Then  页面按顺序展示心动名单、访客、免费悄悄话、额外浏览、高级筛选、曝光、隐私权益、三天回放、每日心动机会，且前台文案使用 UI 名称，不展示“全量查看”总项；每项图标取后台 `mobileIcon` 配置
```
...
| `APP-04-AC-vip-benefit-ui-names` | 会员权益按 UI 名称和配置图标展示 | 正常 | P0 |
...
| 依赖的模块规则 | `M04-RULE-core-access-gate` / `M04-RULE-vip-benefit` / `M04-RULE-member-tier-scope` | 购买与权益 |
| 依赖的模块配置项 | `M04-CFG-vip-benefit-list` / `M04-CFG-vip-package-list` / `M04-CFG-vip-free-whisper-daily` / `M04-CFG-subscription-package-list` | 套餐、权益与移动端图标 |
