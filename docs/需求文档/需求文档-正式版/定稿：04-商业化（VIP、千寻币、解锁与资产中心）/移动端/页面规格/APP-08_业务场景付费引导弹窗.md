...
| 版本 | 日期 | 修改人 | 变更摘要 |
|------|------|--------|----------|
| 版本06 | 2026-07-02 | Codex | 按次扣币场景新增立业-职业推荐，并同步职业测评结果后单次解锁口径 |
| 版本05 | 2026-07-02 | Codex | 按次扣币场景删除觅缘回看单条，新增合拍的人与解锁知音-觅知音，并同步会员不免单口径 |
| 版本04 | 2026-07-02 | Codex | 按一期上线目标移除精选主页付费触点，按次扣币场景改为觅缘回看单条解锁 |
| 版本03 | 2026-07-01 | Codex | 前台付费引导统一为会员引导/会员不免单口径，替换 VIP 引导文案 |
...
- **入口来源**：喜欢我的、最近看过我的、悄悄话、觅缘配额、高级筛选、理想型、合拍的人、知音-觅知音、立业-职业推荐等付费触点
...
| 弹层 | 触发方式 | 大小 | 内容 | 关闭方式 |
|------|----------|------|------|----------|
| 会员引导弹窗 | 解锁全部、配额耗尽、高级筛选等 | 半屏/居中 | 标题、权益摘要、开通按钮、取消按钮 | 取消/遮罩/系统返回 |
| 千寻币引导弹窗 | 单条解锁、悄悄话、理想型、合拍的人、知音-觅知音、立业-职业推荐等 | 半屏/居中 | 所需币数、当前余额、确认扣币/充值按钮 | 取消/遮罩/系统返回 |
...
|---------|--------|------|------|----------|----------|--------|--------|----------|----------|
| `APP-04-PAGE-paywall-modal-FIELD-scene` | 付费场景 | enum | 是 | `M04-ENUM-coin-biz-scene` 或会员场景 | 必须由来源页传入 | 无 | 否 | 普通 | 来源页 |
| `APP-04-PAGE-paywall-modal-FIELD-pay-asset` | 承接资产 | enum | 是 | 时空邂逅会员/千寻币 | 喜欢/访客单条=千寻币，全量=时空邂逅会员 | 无 | 否 | 普通 | `M04-RULE-like-viewer-unlock` |
...
| `APP-04-PAGE-paywall-modal-FIELD-retention-note` | 保留期说明 | string | 否 | 场景规则文案 | 理想型/合拍的人/知音-觅知音/立业-职业推荐展示 | 无 | 否 | 普通 | `M04-CFG-*retention-days` 或测评结果版本 |
...
|---------|--------|------|----------|----------|----------|--------|--------|
| `APP-04-PAGE-paywall-modal-ACT-open-member` | 开通会员 | 会员引导主按钮 | 会员场景 | 已登录 | 否 | 跳 `APP-04-PAGE-vip-center` 并携带来源 | — |
...
| 付费场景 | likes/viewers 全量 | 承接资产 | 固定为时空邂逅会员 | `M04-RULE-like-viewer-unlock` |
...
AC-ID: APP-04-AC-paywall-member-guide-copy
Given 用户触发解锁全部或高级筛选等会员场景
When  打开业务场景付费引导弹窗
Then  弹窗文案使用“会员引导/时空邂逅会员”，不展示“VIP 引导”；理想型、合拍的人、知音-觅知音、立业-职业推荐等按次扣币场景展示会员不免单口径
```
...
| `APP-04-AC-paywall-member-guide-copy` | 会员引导和会员不免单文案 | 正常 | P0 |
...
| 关联类型 | 引用 ID | 说明 |
|----------|---------|------|
| 依赖的模块规则 | `M04-RULE-like-viewer-unlock` / `M04-RULE-whisper-pay` / `M04-RULE-ideal-unlock` / `M04-RULE-compatible-person-unlock` / `M04-RULE-soulmate-mizhiyin-unlock` / `M04-RULE-career-recommend-unlock` | 付费场景 |
