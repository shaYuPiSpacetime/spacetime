...
| 版本 | 日期 | 修改人 | 变更摘要 |
|------|------|--------|----------|
| 版本07 | 2026-07-16 | Codex | 喜欢/访客单条解锁作为第二步确认：扣币前复验对象并保证幂等；取消不扣币 |
...
| `APP-04-PAGE-paywall-modal-ACT-confirm-coin` | 确认支付千寻币 | 千寻币弹窗主按钮 | 余额充足且来源对象再次校验有效 | 已登录 | 是，展示扣减数量 | 幂等扣币成功，来源对象解锁/消息发送 | 余额不足、对象失效、核心准入受限或扣币失败 |
...
| 来源场景 | likes/viewers 单条 | 提交确认 | 再次校验目标记录可解锁，并按用户+场景+目标记录生成幂等键；失效则关闭并刷新来源列表 | 第一步 PRD-02 弹窗不扣币 |
...
| 空态 | 来源对象不存在或已不可展示 | 关闭弹窗并刷新来源列表，不展示失效卡片 | 关闭 | `M02-RULE-relation-invalid` |
...
Given 已登录且核心准入开放的用户，千寻币余额充足，并已从 PRD-02 第一步场景弹窗进入本确认弹窗
When  用户确认且服务端复验目标记录仍可解锁
Then  幂等扣减对应千寻币，写 consume 流水；当前记录在对象与关系可展示时保持清晰

AC-ID: APP-04-AC-paywall-like-one-cancel
Given 用户处于 PRD-02 第一步或本页面第二步确认
When 用户取消、关闭或系统返回
