...
阶梯计算引用 `M07-RULE-invite-ladder`。无风控冻结、无关系作废、无每日封顶。
...
| 需求 ID | 能力 | 页面/视图 | 优先级 |
|---|---|---|---|
| `ADM-07-RULE-rule-config` | 普通邀请/代理双 Tab 奖励配置及阶梯联动 | `ADM-07-PAGE-promo-rule-config` | P0 |
...
| 成功口径、有效期、每日封顶配置 | 代码固定完成注册、永久有效、不封顶，页面无字段 |
