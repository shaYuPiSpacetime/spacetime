...
| 版本 | 日期 | 修改人 | 变更摘要 |
|------|------|--------|----------|
| v0.1 | 2026-06-12 | 皮林雄 | 初稿：基于移动端细化 PRD-07 转正式版 |

---

## 1. 模块目标

承接小程序前期拉新裂变需求，首版纳入正式范围。目标不是短期盈利，而是快速做注册增长与基础活跃增长。模块分两条业务线：普通用户邀请裂变（分享 → 注册登录/头像认证/实名+学历/首次会员/首次充值等节点按后台规则发成家币 → 成功邀请统计节点由后台配置）、校园代理推广承接（仅做来源识别与统计入口，奖金走后台线下结算）。

**用户故事：** 作为已注册的普通用户，我想把小程序分享给同学并在他们完成活动规则要求的节点后拿到成家币奖励，以便基本免费体验核心玩法。
...
| M07-08 | 邀请页“成家币能干嘛”对齐 PRD-04，本模块不单独定死用途 | `APP-07-PAGE-invite-home` |
...
正常路径：
  ...
  6. 被邀请人首次充值成家币 → 触发首次充值奖励（若后台启用且金额已配置）
...
| 成家币流水 | `asset_coin_flow` | 奖励到账写入 | 04 | userId, type(invite_*_reward), amount |
| 支付事件 | PRD-04 资产/订单 | 首次会员、首次充值成家币奖励触发依据 | 04 | userId, orderType, paidTime, firstPaidFlag |
...
| 成家币余额/流水 | 04 商业化 | 资产中心 | 必须依赖，奖励到账写流水 |
...
| 成家币资产 | PRD-04 商业化 | 奖励到账写流水 | 必须就绪 | 阻塞 |
...
| 成家币资产流水 | 新增 5 类 invite_* 流水类型（注册、资料完善、认证完成、首次会员、首次充值） | 是 | — |
