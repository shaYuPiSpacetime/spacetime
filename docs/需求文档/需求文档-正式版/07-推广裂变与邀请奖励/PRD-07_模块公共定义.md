...
|------|------|--------|----------|
| v0.1 | 2026-06-15 | Codex | 从全局共享层迁入 PRD-07 专属定义，统一改用 `M07-*` 命名空间；影响 APP-07、ADM-07 全部页面 |
...
|---------|----------|--------------|------|------|
| `M07-TERM-inviter` | 邀请人 | 推荐人 | 已注册的普通小程序用户，生成个人邀请二维码邀请新用户 | 角色 `Inviter` |
| `M07-TERM-invitee` | 被邀请人 | 受邀人 | 通过普通用户或校园代理二维码进入小程序的新用户 | 角色 `Invitee` |
| `M07-TERM-campus-agent` | 校园代理 | 渠道商、代理商 | 后台录入与管理的合作代理人，以奖金/佣金结算，不走普通用户发币逻辑 | 角色 `CampusAgent` |
| `M07-TERM-success-invite` | 成功邀请 | 有效邀请 | 关系有效且达到后台配置的成功统计节点后，计为一次成功邀请 | 普通+代理统一取同一后台统计口径 |
| `M07-TERM-auth-complete` | 认证完成 | 认证达标 | 实名认证 + 学历认证均完成，用于 PRD-07 认证完成奖励 | 头像认证归入资料完善奖励 |

### 1.1 引用的全局术语

| 全局术语 ID | 统一术语 | 使用场景 |
|-------------|----------|----------|
| `GLB-TERM-cheng-coin` | 成家币 | 邀请奖励金额、成家币流水、奖励到账通知 |
...
| 奖励币名称 | 正式文档统一 `成家币`，不出现 `千寻币`；术语引用 `GLB-TERM-cheng-coin` |
...
| `first_coin_recharge_reward` | 首次充值奖励 | 被邀请人首次充值成家币后发放，流水类型 `invite_first_coin_reward` | 5 | 否 | 启用 |
...
| `success` | 已到账 | 奖励已发放并写入成家币流水 | 2 | 否 | 启用 |
...
| `success` | 已到账 | 已写入成家币流水 | 查看流水 | 查看 | 是 |
...
|----------|-----------|----------|----------|--------|
| （无） | 触发条件命中且未命中风控 | `success` | 满足对应奖励事件且未触发风控 | 写成家币流水，发 `M07-NTF-invite-reward-arrive`，埋点 `invite_reward_arrive` |
...
| `M07-RULE-agent-paid-attribution` | 代理付费归因 | 07/04 | 用户通过代理二维码建立关系后，首次成为会员、首次充值成家币继续归因给原代理；关系永久有效；是否结算与金额由后台代理奖励规则配置；不做续费、复购、长期消费分成 | A07-06 已确认 |
...
|---------|----------|------|-----------|-----------|-------------|------|
| `M07-NTF-invite-reward-arrive` | `M07-EVT-invite-reward-success` | 站内/微信订阅 | 邀请成功奖励到账 | 你邀请的好友{nickname}已{event}，奖励{amount}成家币已到账 | 是 | |
...
| `M07-TXT-invite-entry-sub` | 免费获得成家币 | 我的页入口副文案 | 是 | APP |
| `M07-TXT-invite-home-title` | 邀请好友得成家币 | 邀请首页标题 | 是 | APP/ADM 配置 |
| `M07-TXT-invite-home-subtitle` | 邀请好友完成活动任务，成家币奖励到账 | 邀请首页副标题 | 是 | APP/ADM 配置 |
...
| 奖励金额单位 | 成家币（整数枚），与 PRD-04 资产口径一致 |
