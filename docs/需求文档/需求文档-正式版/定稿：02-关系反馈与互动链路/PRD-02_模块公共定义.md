...
| 版本 | 日期 | 修改人 | 变更摘要 |
|------|------|--------|----------|
| 版本12 | 2026-08-07 | Codex | 对齐 PRD-03/TIM 技术边界：本地匹配、会话、悄悄话状态同事务，TIM 通过幂等 Outbox 保证用户可见的全成或全败 |
| 版本11 | 2026-07-31 | Codex | 对齐 PRD-03 悄悄话回复原子提交：回复消息、匹配关系、普通会话与悄悄话状态必须全成或全败，提交后再发布匹配事件 |
...
| M02-05 | 悄悄话回复可作为匹配成功来源之一；PRD-03 编排一次幂等业务命令，PRD-02 在同一 MySQL 事务内生成匹配记录、普通会话并更新悄悄话状态；TIM 回复通过可靠 Outbox 投递和最终确认，业务事务提交前不得开放聊天入口、展示回复或计入有效私信未读，对用户保持全成或全败 | `M02-ENUM-match-source`、`M02-RULE-whisper-match` |
...
| `M02-RULE-whisper-match` | 悄悄话回复触发匹配 | APP/ADM | PRD-03 以 `whisperId + replyRequestId` 编排幂等回复命令：TIM 回复由可靠 Outbox 投递；确认投递后，PRD-02 匹配来源、普通会话、TIM 映射和悄悄话 `replied` 状态在同一 MySQL 事务内提交。事务提交前客户端不得展示回复、会话、匹配或有效私信未读；TIM 已送达而本地提交暂失败时继续最终确认，不重复发送；TIM 永久失败时保持/恢复 `pending`。提交成功后再发布 `M02-EVT-match-created`，接口才返回成功 | 第三方 TIM 不参与数据库物理事务，通过幂等、隐藏半成品和补偿实现用户可见的全成或全败 |
...
| `M02-PARAM-whisper-reply-match-enabled` | 悄悄话回复是否触发匹配成功 | true | bool | 后端代码常量/原子回复命令 | 否 | 悄悄话发送和回复归 PRD-03，不使用异步事件补建匹配 |
...
| `M02-EVT-match-created` | 事件 | 匹配事务提交成功 | 内部事件 | matchNo, userAId, userBId, matchSource, sourceEventNo | 否 |
...
| PRD-03 消息、私信与通知中心 | 生成匹配成功、弹窗状态和 `canEnterConversation` | 会话、悄悄话发送/回复、`canSend`、`protectStatus`、TIM 传输及消息主表明文归档；日常消息不做发送前内容审核 |
