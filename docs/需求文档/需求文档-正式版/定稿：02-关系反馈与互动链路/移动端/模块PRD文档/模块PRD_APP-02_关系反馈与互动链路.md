...
| 版本 | 日期 | 修改人 | 变更摘要 |
|------|------|--------|----------|
| 版本08 | 2026-08-07 | Codex | 对齐 TIM 外部投递边界：本地业务事实同事务，Outbox 最终确认前不向用户暴露半完成状态 |
| 版本07 | 2026-07-31 | Codex | 对齐 PRD-03 悄悄话回复原子提交：回复、匹配、会话创建与状态更新全成或全败 |
...
```text
入口：接收方在 PRD-03 提交悄悄话回复
正常路径：
  1. PRD-03 发送悄悄话，不要求双方已普通匹配
  2. PRD-03 以悄悄话 ID 和请求幂等键创建可靠回复 Outbox；TIM 投递失败时保持/恢复 `pending`
  3. TIM 确认投递后，同一 MySQL 事务内由 PRD-02 幂等生成 `matchSource=whisper_reply` 的匹配及来源记录、创建或恢复普通会话、写入 TIM 映射并将悄悄话置为 `replied`
  4. 本地事务提交前不展示回复消息、匹配、普通会话或有效私信未读；TIM 已送达而事务暂失败时按同一幂等键继续最终确认，不重复发消息
  5. 全部提交成功后，为双方分别生成匹配弹窗待展示状态并发布匹配成功事件，双方进入相互喜欢列表；接口此时才返回成功
