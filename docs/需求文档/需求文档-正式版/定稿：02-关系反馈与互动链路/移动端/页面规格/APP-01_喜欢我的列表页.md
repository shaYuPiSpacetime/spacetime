...
| `APP-02-PAGE-likes-me-FIELD-read-cursor` | 本次读取快照游标 | string | 条件必填 | 服务端不透明字符串 | `newCount>0` 时返回；仅提交给已读接口，客户端不得解析、拼接或比较 | 无 | 否 | 普通 | PRD-02 |
| `APP-02-PAGE-likes-me-FIELD-new-preview-avatars` | 新喜欢头像摘要 | image[] | 条件必填 | 0-5 个图片 URL/模糊占位 | `newCount>0` 时按喜欢时间倒序返回最多 5 个；逐条服从 `M02-RULE-blur-display` | 空数组 | 否 | 敏感；未解锁时仅返回模糊图，不导出、不本地持久化，账号注销或对象失效后不可见 | PRD-01 用户资料、PRD-02 展示状态 |
...
- **默认排序**：先按“新喜欢/更早”分组，组内按喜欢时间倒序；由于新喜欢来自当前读取游标之后，正常情况下同时满足全局喜欢时间倒序。
- **普通用户**：`total` 返回当前有效喜欢真实总数；列表仅返回最近 10 条模糊记录，不提供第 2 页。
- **会员/全量清晰权益有效**：移动端加载更多，默认每页 20 条。
...
When  喜欢我的页面首屏渲染完成
Then  入口角标显示 5，顶部显示“5 个新喜欢”和最多 5 个合规头像摘要，列表按“新喜欢/更早”分组；模糊态只显示模糊头像、喜欢时间和“新”标签，不泄露昵称、年龄或学校
