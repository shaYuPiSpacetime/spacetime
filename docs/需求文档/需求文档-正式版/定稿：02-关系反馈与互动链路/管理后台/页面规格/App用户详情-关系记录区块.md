# 页面规格 - ADM-02-PAGE-user-relation-section App 用户管理模块补充弹窗-关系反馈 Tab
...
- **入口来源**：用户管理中心 -> App 用户管理 -> 用户卡片“模块补充” -> 关系反馈 Tab
...
- **目标用户**：客服、运营、风控、超级管理员
- **核心任务**：在模块补充弹窗中查看该用户的关系反馈摘要与明细，解释前台展示、失效和解锁问题
- **页面类型**：原页面卡片入口弹窗 / Tab
...
|------|------|------|------------|------------------|
| 关系反馈摘要 | 模块补充弹窗-关系反馈 Tab 顶部 | 核心准入、统计、最近匹配时间 | 否 | 否 |
| 喜欢记录 Tab | 摘要下方 | 用户作为发起方/接收方的喜欢记录 | 否 | 是 |
| 访客记录 Tab | 摘要下方 | 用户作为访问者/被访问者的访客记录 | 否 | 是 |
| 相互喜欢 Tab | 摘要下方 | 用户参与的相互喜欢记录 | 否 | 是 |
| 解锁记录 Tab | 摘要下方 | 与喜欢/访客相关的解锁记录，资产流水跳 PRD-04 | 否 | 是 |
...
本 Tab 嵌入单个用户的模块补充弹窗，不提供全局搜索；用户定位由 App 用户管理卡片承接。
...
| 关系状态 | 失效 | 失效原因 | 明细表显示真实失效原因；前台按默认隐藏/历史入口通用提示策略处理 | `M02-RULE-relation-invalid`、`M02-RULE-like-cancel` |
...
Given 客服打开某用户详情
When  从用户卡片点击“模块补充”并进入“关系反馈”Tab
