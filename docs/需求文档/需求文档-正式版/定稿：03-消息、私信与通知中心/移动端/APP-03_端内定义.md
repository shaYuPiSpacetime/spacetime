...
| 版本 | 日期 | 修改人 | 变更摘要 |
|------|------|--------|----------|
| 版本05 | 2026-08-12 | Codex | 私信列表合并进消息首页，页面树收口为 6 个有效页面 |
| 版本04 | 2026-08-07 | Codex | 页面树收口为 7 个有效页面，历史通知详情和邀请响应仅保留废弃登记 |
...
| `APP-03-PAGE-private-list` | 私信列表页（已合并废弃） | 蓝湖补充 | - | `页面规格/APP-09_私信列表页.md` | ID 保留，不提供独立入口 |
...
| 官方消息详情页 | 原 MVP-PAGE-036 / APP-PAGE-052 | 隐藏，历史链接统一回系统消息全文流并定位合法消息 | 避免形成已废弃的二级详情 |
| 图片/语音/通话输入入口 | 原 PRD-03 可延后项 | 隐藏 | 首版只做文本、悄悄话与系统提示 |
| 黑名单管理页 | 一期边界 | 隐藏 | 聊天页保留拉黑动作，不做完整黑名单管理页 |
| 独立私信列表页 | 最新确认口径 | 合并到消息首页 | 首页直接游标分页全部有效普通私信 |
