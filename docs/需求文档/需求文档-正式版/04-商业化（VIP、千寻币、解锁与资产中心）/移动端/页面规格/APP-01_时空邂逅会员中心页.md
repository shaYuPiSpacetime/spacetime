...
- **目标用户**：已登录用户
- **核心任务**：查看时空邂逅会员状态、理解会员权益、选择普通套餐或连续订阅套餐并完成开通/续费；首版不展示 SVIP/高端服务套餐
- **页面类型**：购买页
...
| 权益列表 | 中部 | `M04-CFG-vip-benefit-list` 返回的 9 项权益，前台名称按 UI 展示 | 否 | 否 |
| 套餐区 | 中部 | 普通套餐、连续订阅套餐卡片；不展示 SVIP/高端服务套餐 | 否 | 是 |
...
| `APP-04-PAGE-vip-center-FIELD-benefits` | 权益列表 | json | 是 | `M04-CFG-vip-benefit-list` | 仅展示启用权益；固定顺序展示心动名单、访客、免费悄悄话、额外浏览、高级筛选、曝光、隐私权益、三天回放、每日心动机会 | 空列表 | 否 | 普通 | 后台商业化配置 |
| `APP-04-PAGE-vip-center-FIELD-package-type` | 套餐类型 | enum | 是 | `M04-ENUM-vip-package-type` | 仅允许普通套餐/连续订阅套餐；连续订阅商品不可用时置灰 | `normal` | 用户切换 | 普通 | 后台商业化配置 |
...
| 展示顺序 | 权益 code | 前台 UI 名称 | 后台能力名 | 展示值来源 | 说明 |
|----------|-----------|--------------|------------|------------|------|
| 1 | `heart_list` | 心动名单一键揭晓 | 心动名单 | 开关 | 原全量查看拆分项 |
| 2 | `visitor_list` | 谁来看过你 / 访客全公开 | 访客 | 开关 | 原全量查看拆分项 |
| 3 | `free_whisper` | 每日专属悄悄话 | 免费悄悄话 | 开关 + `M04-CFG-vip-free-whisper-daily` | 用完后继续走千寻币 |
| 4 | `extra_browse` | 每日额外浏览 | 额外浏览 | 开关 + `M04-CFG-vip-extra-browse-daily` | 次数由后台配置 |
| 5 | `advanced_filter` | 精准筛选功能 | 高级筛选 | 开关 | 前台按 UI 展示“精准筛选” |
| 6 | `exposure_score` | 曝光度拉满 | 曝光 | 开关 + `M04-CFG-vip-exposure-score` | 分数由后台配置 |
| 7 | `privacy` | 隐身模式 | 隐私权益 | 开关 | 前台按 UI 展示“隐身” |
| 8 | `three_day_replay` | 三天回放功能 | 三天回放 | 开关 | 只展示是否可用 |
| 9 | `daily_heart_chance` | 每日心动机会 | 每日心动机会 | 开关 + `M04-CFG-vip-daily-heart-chance` | 次数由后台配置 |
...
| 套餐类型 | 出现 SVIP/高端服务 | 套餐区 | 不展示该类型，视为配置不合法并提示刷新套餐 | `M04-RULE-member-tier-scope` |
...
When  用户进入时空邂逅会员中心
Then  页面按顺序展示心动名单、访客、免费悄悄话、额外浏览、高级筛选、曝光、隐私权益、三天回放、每日心动机会，且前台文案使用 UI 名称，不展示“全量查看”总项
```
...
| `APP-04-AC-vip-benefit-ui-names` | 会员权益按 UI 名称展示 | 正常 | P0 |
