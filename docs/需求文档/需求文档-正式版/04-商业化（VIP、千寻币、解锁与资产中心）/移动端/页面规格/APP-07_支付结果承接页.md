...
|---------|--------|------|----------|----------|----------|--------|--------|
| `APP-04-PAGE-payment-result-ACT-back-source` | 返回来源页 | 主按钮 | 有来源页 | 已登录 | 否 | 回到来源页并刷新权益/余额 | 来源页不可用则回资产中心 |
| `APP-04-PAGE-payment-result-ACT-go-asset` | 去资产中心 | 次按钮 | 已登录 | 已登录 | 否 | 跳 `APP-04-PAGE-asset-center` | — |
...
| 订单状态 | `closed` | 结果标题 | 展示支付已取消/已关闭，资产不变 | |
...
| 空态 | 订单不存在 | 错误说明 | 返回资产中心 | `M04-ERR-order-not-found` |
| 错误态 | 查询失败 | toast + 重试 | 重试 | — |
| 无权限态 | 订单不属于当前用户 | 无权限提示 | 返回 | — |
| 业务态-成功 | `orderStatus=success` | 成功标题 + 获得资产 | 回来源/资产中心 | `M04-SM-trade-order` |
...
| 依赖的其他页面 | `APP-04-PAGE-asset-center` | 跳转 |
