...
- **入口来源**：喜欢我的、最近看过我的、悄悄话、觅缘配额、高级筛选、理想型、觅缘回看等付费触点
...
| 千寻币引导弹窗 | 单条解锁、悄悄话、理想型、觅缘回看等 | 半屏/居中 | 所需币数、当前余额、确认扣币/充值按钮 | 取消/遮罩/系统返回 |
...
| `APP-04-PAGE-paywall-modal-FIELD-retention-note` | 保留期说明 | string | 否 | 场景规则文案 | 理想型/觅缘回看展示 | 无 | 否 | 普通 | `M04-CFG-*retention-days` |
...
When  打开业务场景付费引导弹窗
Then  弹窗文案使用“会员引导/时空邂逅会员”，不展示“VIP 引导”；理想型、觅缘回看等按次扣币场景展示会员不免单口径
```
...
| 关联类型 | 引用 ID | 说明 |
|----------|---------|------|
| 依赖的模块规则 | `M04-RULE-like-viewer-unlock` / `M04-RULE-whisper-pay` / `M04-RULE-ideal-unlock` / `M04-RULE-fate-replay-unlock` | 付费场景 |
