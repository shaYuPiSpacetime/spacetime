...
| 消费场景说明区 | 中部 | 后台配置的 6 个千寻币消费场景 | 否 | 否 |
...
| `APP-04-PAGE-coin-recharge-FIELD-use-scenes` | 千寻币消费场景 | json | 是 | `M04-CFG-coin-scene-price` 的 6 个消费场景 | 仅展示启用消费场景；不展示 `invite_*` 奖励场景和积分用途 | 6 个消费场景 | 否 | 普通 | 后台商业化配置 |
...
| 消费场景 | 加载配置 | 用途说明区 | 仅展示 `whisper`、`likes_unlock_one`、`viewers_unlock_one`、`ideal_user_unlock`、`ideal_batch_unlock`、`featured_profile_unlock` 6 个消费场景 | `M04-RULE-coin-consume-price-scope` |
...
Then  阻断支付并提示勾选协议；页面用途说明只展示后台配置的 6 个千寻币消费场景
```
...
| `APP-04-AC-coin-agreement-and-scenes` | 协议勾选与消费场景收口 | 正常 | P0 |
