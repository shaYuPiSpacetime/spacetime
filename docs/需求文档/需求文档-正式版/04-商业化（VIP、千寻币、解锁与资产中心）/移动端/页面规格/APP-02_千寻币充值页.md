...
| 版本 | 日期 | 修改人 | 变更摘要 |
|------|------|--------|----------|
| 版本07 | 2026-07-02 | Codex | 千寻币消费场景新增立业-职业推荐，消费场景扩展为后台配置 8 项 |
| 版本06 | 2026-07-02 | Codex | 千寻币消费场景改为后台配置 7 项，并新增移动端展示名称取值口径 |
...
- **入口来源**：我的页「千寻币」；余额不足弹窗；单条解锁/悄悄话/理想型/合拍的人/知音-觅知音/立业-职业推荐付费触点
...
| 消费场景说明区 | 中部 | 后台配置的 8 个千寻币消费场景，含移动端展示名称、移动端图标和单价 | 否 | 否 |
...
| `APP-04-PAGE-coin-recharge-FIELD-use-scenes` | 千寻币消费场景 | json | 是 | `M04-CFG-coin-scene-price` 的 8 个消费场景 | 仅展示启用消费场景；每项必须返回 `mobileDisplayName` 和 `mobileIcon`；不展示 `invite_*` 奖励场景和积分用途 | 8 个消费场景 | 否 | 普通 | 后台商业化配置 |
...
| 消费场景 | 加载配置 | 用途说明区 | 仅展示 `whisper`、`likes_unlock_one`、`viewers_unlock_one`、`ideal_user_unlock`、`ideal_batch_unlock`、`compatible_person_unlock_one`、`soulmate_mizhiyin_unlock_one`、`career_recommend_unlock_one` 8 个消费场景；展示名称取 `M04-CFG-coin-scene-price.mobileDisplayName`，图标取 `M04-CFG-coin-scene-price.mobileIcon` | `M04-RULE-coin-consume-price-scope` |
...
Then  阻断支付并提示勾选协议；页面用途说明只展示后台配置的 8 个千寻币消费场景，且每个场景展示名称取后台 `mobileDisplayName`、图标取后台 `mobileIcon` 配置
```
...
| 依赖的模块配置项 | `M04-CFG-coin-package-list` / `M04-CFG-coin-scene-price` | 套餐、消费场景移动端展示名称与图标 |
