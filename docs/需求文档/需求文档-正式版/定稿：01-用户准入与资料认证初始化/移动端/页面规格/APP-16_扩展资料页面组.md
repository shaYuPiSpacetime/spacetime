...
| `APP-01-PAGE-profile-extended-edit-ACT-save-text` | 保存开放文本 | 长度符合配置 | 生成开放性文字审核记录；本人侧展示审核中 | 校验失败提示 |
| `APP-01-PAGE-profile-extended-edit-ACT-save-tags` | 保存标签 | 标签数量符合配置 | 保存并刷新完整度 | 超数量提示 |
| `APP-01-PAGE-profile-extended-edit-ACT-upload-photo` | 上传相册 | 选择图片 | 单张照片生成一条资料图片审核记录；本人侧展示审核中 | 上传失败可重试 |
| `APP-01-PAGE-profile-extended-edit-ACT-upload-bg` | 上传资料背景图 | 选择图片 | 背景图生成一条资料图片审核记录；本人侧展示审核中 | 上传失败可重试 |
| `APP-01-PAGE-profile-extended-edit-ACT-record-voice` | 录制/上传语音 | 权限允许且时长 10-60 秒 | 生成语音介绍审核记录并提交微信音频机审；本人侧展示审核中 | 权限拒绝/机审不通过提示重录 |
...
| `APP-01-PAGE-profile-extended-edit-ACT-save-qa` | 保存资料问答 | 题目和回答有效 | 生成开放性文字审核记录；本人侧展示审核中 | 校验失败提示 |
...
3. 资料背景图为独立字段，不并入相册、不参与 3 张相册计数；新图通过前旧图继续展示，新图通过后替换旧图，当前生效图失效后不自动回退旧图。
4. 相册审核与主头像认证隔离，一张照片一条审核记录；通过一张展示一张，驳回/失效只影响该张照片，不影响其他已通过照片。
5. 关于我、资料问答等开放文本进入开放性文字审核；新内容通过前旧内容继续展示，驳回/失效原因原文展示，当前生效内容失效后不自动回退旧内容。
6. 语音介绍走微信音频内容安全机审，首版不设后台人工语音审核页；语音不转文字、不进入开放性文字审核，新语音通过前旧语音继续播放，驳回/失效不展示新语音。
...
|----------|------|----------|
| 空态 | 未填写扩展资料 | 展示补充入口 |
| 待审核/审核中 | 文本/图片/问答/语音提交后 | 本人侧统一展示审核中；他人端按最近已通过且有效内容展示，语音未通过前不展示新语音 |
| 已通过 | 文本/图片/问答/语音通过 | 对本人和他人端展示通过后的内容；相册单张追加展示，背景图替换旧图 |
| 已驳回 | 文本/图片/问答/语音被驳回 | 本人侧展示原文原因并允许修改/重传；他人端继续展示最近已通过且有效内容，若无则不展示 |
| 已失效 | 文本/图片/问答/语音被标记失效 | 本人侧展示失效原因并允许重传；当前生效内容失效后不自动回退旧内容 |
...
| `APP-01-AC-extended-voice-safety` | 语音介绍机审通过后展示播放器，机审失败/失效可重录且不做转文字 | 安全 | P1 |
