...
| 截图证据 | `截图证据/` | 37 张有效 PNG，覆盖默认页、关键弹窗、异常态、抽屉和交互结果。 |
...
| Chrome 打开与页面截图 | 通过 | Playwright 输出 `defaultScreenshots=14`、`effectiveScreenshots=37`、`errors=[]`。 |
| 配置页字段断言 | 通过 | 会员权益固定 9 项，支持移动端图标配置；免费悄悄话/额外浏览/每日心动机会为次数输入，曝光为分数输入；会员套餐无千寻币类型；千寻币套餐固定类型；6 个千寻币消费场景存在且支持移动端图标配置。 |
...
- 评审时重点确认会员权益只读字段与移动端图标配置、免费悄悄话次数、千寻币消费场景图标/单价配置、会员套餐与千寻币套餐字段、订单详情退款、退款记录只读台账。
