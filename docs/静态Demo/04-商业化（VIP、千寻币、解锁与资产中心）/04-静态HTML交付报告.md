...
| 移动端入口 | `docs/静态Demo/04-商业化（VIP、千寻币、解锁与资产中心）/html/miniapp.html` | 手机外框模拟 8 个 APP 页面和付费弹窗链路。 |
| 管理后台入口 | `docs/静态Demo/04-商业化（VIP、千寻币、解锁与资产中心）/html/admin.html` | 后台骨架模拟 6 个 ADM 页面，左侧菜单按 hash 隔离工作台。 |
...
| `APP-04-PAGE-asset-center` | 资产中心页 | `截图证据/mini-APP-04-PAGE-asset-center-default.png` | 已实现 |
...
| `ADM-04-PAGE-user-asset-detail` | 用户商业化详情页 | `截图证据/admin-ADM-04-PAGE-user-asset-detail-default.png` | 已实现 |
...
| 套餐编辑与 6 个消费场景 | `admin-FLOW-admin-package-edit-modal.png`、`admin-ADM-04-PAGE-commerce-config-scene-prices.png` | 已验证 |
| 用户资产排查和人工修正 | `admin-FLOW-admin-user-asset-flow-detail.png`、`admin-STATE-admin-asset-adjust-modal.png` | 已验证 |
| 订单详情和导出 | `admin-FLOW-admin-order-detail-drawer.png`、`admin-FLOW-admin-order-export-modal.png` | 已验证 |
...
| GAP-05 | 导出、对账、退款渠道状态均为静态提示。 | P1 | 不能验证真实文件生成和渠道延迟。 | 后端任务、审计和渠道同步完成后补端到端验收。 |
...
| Chrome 打开与默认页截图 | 通过 | Playwright 输出 `screenshots=18`、`errors=[]`。 |
| 关键弹窗/抽屉/异常态截图 | 通过 | Playwright 输出 `screenshots=37`、`extraScreenshots=19`、`errors=[]`。 |
| 退款只读断言 | 通过 | Playwright 输出 `refundReadonly=true`。 |
| 6 个消费场景断言 | 通过 | Playwright 输出 `sceneCount=6`。 |

## 8. 后续建议

- 基于 `html/index.html` 组织产品、需求方、技术评审。
- 评审时重点确认套餐配置、6 个消费场景单价、连续订阅真实接入、退款只读边界、后台人工修正权限。
