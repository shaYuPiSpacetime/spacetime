...
**架构：** 使用腾讯云 IM 作为实时消息和历史漫游基础设施；本项目后端作为业务规则、鉴权、扣费、回调、审计和后台查询的权威层。小程序首选无 UI SDK，页面和交互由现有 Taro 组件自绘。

**技术栈：** Taro 4.1.9、React 18、TypeScript、腾讯云 IM 小程序/uni-app 无 UI SDK、Spring Boot 3.4、Java 21、MyBatis-Plus、MySQL、Redis。
...
- 创建 `miniapp/src/services/im.ts`：IM SDK 初始化、登录、登出和事件监听。
- 创建 `miniapp/src/services/message.ts`：消息业务接口封装。
- 创建 `miniapp/src/hooks/useImSession.ts`：小程序登录态和 SDK 生命周期。
...
- 创建 `backend/src/main/java/com/spacetime/common/entity/AppUserImAccount.java`、`AppImConversation.java`、`AppImMessage.java`、`AppImWhisper.java`、`AppImNotification.java`：本地业务数据。
...
**目标：** 验证当前 Taro 4.1.9 能否稳定完成 IM 初始化、登录、两账号收发、历史拉取和断线重连。

- [ ] 记录 SDK 版本、微信开发者工具版本、基础库版本和构建结果。
- [ ] 使用后端临时 UserSig 接口，不在前端写 SecretKey。
- [ ] 验证 socket/request 合法域名、SDK_READY、被踢下线和 UserSig 过期事件。
...
- [ ] 编写回调控制器测试，覆盖错误 SDKAppID、未知 IM UserID、允许发送、拒绝发送和重复消息回调。
- [ ] 创建会话、消息、通知和审计表，增加 IM 消息唯一索引。
- [ ] 消息前回调校验账号状态、认证、匹配、女性保护、拉黑和封禁。
- [ ] 消息后回调以 MsgKey/MsgId 幂等落库并更新未读摘要。
...
- [ ] 实现幂等键贯穿资格校验、支付、IM 发送和补偿。
- [ ] 使用自定义消息字段标识 `messageType=whisper`、`whisperId` 和 `conversationId`。
