...
- 蓝湖切图落地时必须先自动转 WebP，再进入 `miniapp/src/assets`；除 Tab 图标、小于 `4K` 的线性图标、确需透明像素且 WebP 体积不优的极小 PNG 外，运行时代码不得新增 `.png` / `.jpg` 大图。
- 新切图必须保留两份：原始 PNG/JPG 放 `miniapp/.lanhu-ref/` 或 `miniapp/.lanhu-ref/compressed-source-backup/`，运行时 WebP 放 `miniapp/src/assets` 并通过本地 import 引入。
- 新增 WebP 时必须保留 `miniapp/types/global.d.ts` 中的 `declare module '*.webp'`。
...
前两条和 PNG/JPG 大图扫描必须无输出；主包估算必须低于 `1.5M`，`main-assets-kib` 与 `all-assets-kib` 都必须低于 `200K`。若 `src` 通过但 `dist` 未通过，说明开发者工具读取的是旧编译产物，必须清理 `dist` 或重新生成上传包。
...
Step 3：局部切图先归档原图，再自动转 WebP 放入 src/assets
Step 4：对照蓝湖标注逐项翻译为代码
Step 5：用 Tailwind 设计 Token（非任意值）
Step 6：编译验证 → 对照设计稿截图核对
```
...
| 新切图 PNG/JPG 直接放 `src/assets` | 原图进 `.lanhu-ref`，运行图自动转 WebP 后再 import |
